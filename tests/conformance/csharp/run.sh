...
# format does not have -- and generated C# could not notice, since its array
...
python3 "$ROOT/tests/conformance/lib/check_fixlen_array_subtype.py" "csharp" \
...
# the cap sits inside the `case <id>:` arm behind the ArrayKind test. Hoist it to
# the top of the callback, or drop the kind test, and both rows answer
# LimitExceeded while every assertion above still passes -- the cap is still
# present and still fires on the control. TryDecode reads the CATEGORY (6.3)
...
for v in skipmistyped skipunknown; do
    ...
    # resized from the skipped header's count (7.3, 7.4).
...
# MATCHING kind is this field's own count and stays the policy category. A
