...
BASE=$(printf '%s' '{}' | "$WORK/ex/zig-out/bin/harness" encode myfirstmessage | "$WORK/ex/zig-out/bin/harness" decode myfirstmessage)
python3 "$ROOT/tests/conformance/lib/check_nondefault.py" "$BASE" "$FULL" \
    --except '$.someunion.option2,$.someunion.option3,$.someunion.option3.unionstructint,$.someunion.option3.unionstructbool' --label "zig: round-trip fixture" || exit 1
