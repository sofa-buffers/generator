#!/usr/bin/env python3
"""Drive the generated Zig harness against the shared wire vectors (byte-exact).

Usage: check_vectors.py <test_vectors.json> <project-dir>
Runs `zig-out/bin/harness encode <message>` for each single-field id-0 scalar
vector and compares the hex output byte-for-byte to the vector's
`serialized_sparse` — the sparse-canonical bytes a generated encoder must
produce (MESSAGE_SPEC S2): empty for a default-valued field, else the dense
bytes.
"""
import json
import os
...
def string_array_values(fields):
    ...
    whose children are all string ops (a wrapper-array of string) — the shape
    the MESSAGE_SPEC S2 element-omission vectors use; else None. Encoded against
    the `vecsa` harness message."""
...
def main() -> int:
    vectors_path, project = sys.argv[1], sys.argv[2]
    harness = os.path.join(project, "zig-out", "bin", "harness")
    ...
    for v in data["vectors"]:
        ...
        if got != want:
            ...
            return 1
        checked += 1
    print(f"Zig shared-vector conformance: {checked} byte-exact")
