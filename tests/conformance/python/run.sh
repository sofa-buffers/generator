...
for ENGINE in $ENGINES; do
    ...
    # MESSAGE_SPEC §4.6, generator#414). A Python float is a double, and the
    # usual way to widen an fp32 signaling NaN into one QUIETS it
    # (0x7F800001 -> 0x7FC00000), so decode -> re-encode stops reproducing the 4
    # wire bytes. TypeScript (#235) and Dart (#226) -- the other two double-only
    # targets -- each pin this after a real defect; python had no NaN case at all.
    #
    ...
    # ts and dart emit one). The JSON detour through `decode | encode` is
    # measurably lossy -- an sNaN comes back as the canonical quiet NaN -- so a
    # block built on it would be vacuous or falsely red. The driver also drives
    # the STREAMING surface with the same fixtures, which §6.5 asks for
    # ("on every decode surface") and neither the ts nor the dart block covers.
