#!/usr/bin/env python3
"""Drive the generated Kotlin harness against the shared wire vectors (byte-exact).

Usage: check_vectors.py <test_vectors.json> <harness-bin>
Runs `<harness-bin> encode <message>` for each single-field id-0 scalar
...
def main() -> int:
    ...
    for v in data["vectors"]:
        ...
        if got != want:
            ...
            return 1
        checked += 1
    print(f"Kotlin shared-vector conformance: {checked} byte-exact")
