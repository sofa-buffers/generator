...
func TestGoOverIndexWrapperArray(t *testing.T) {
	...
	for _, want := range []string{
		...
		"sofab.NewStringSeq(&m.Bs, sofab.Bounds{Count: 4, ElemLen: 16}, _caps)",
		...
		"sofab.NewBlobSeq(&m.Bb, sofab.Bounds{Count: 3, ElemLen: 16}, _caps)",
		// bounded struct -> sofab.MessageSeq Cap 2, index cap inert beside it
		"(&m.Bp, sofab.Bounds{Count: 2}, _caps)",
...
func TestGoMaxlenReject(t *testing.T) {
	...
	for _, want := range []string{
		...
		"sofab.NewStringSeq(&m.Ws, sofab.Bounds{ElemLen: 5}, _caps)",
...
`)
	...
	if !strings.Contains(got, "sofab.Bounds{Count: 5}") || !strings.Contains(got, "if count > 4 {") {
...
func TestGoSkippedStringIsNotValidated(t *testing.T) {
	...
	if !strings.Contains(msg, "sofab.NewStringSeq(&m.Sa, sofab.Bounds{Count: 4}, _caps)") {
...
func TestGoNestedRowElemWidth(t *testing.T) {
	...
	for _, want := range []string{
		"sofab.NewUnsignedMatrixSeq[uint8](&m.Urows, sofab.Bounds{Count: 2}, sofab.Bounds{Count: 3}, _caps, 255)",
		"sofab.NewSignedMatrixSeq[int16](&m.Srows, sofab.Bounds{Count: 2}, sofab.Bounds{Count: 3}, _caps, -32768, 32767)",
		...
		"sofab.NewUnsignedMatrixSeq[uint64](&m.Wide, sofab.Bounds{Count: 2}, sofab.Bounds{Count: 3}, _caps, 0)",
	} {
		if !strings.Contains(msg, want) {
