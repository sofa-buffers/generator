...
func (*Backend) Generate(s *ir.Schema, cfg map[string]any) ([]generator.File, error) {
	...
	for _, m := range s.Messages {
		g.schemaFile = nil
		g.schemaNeededOrder = nil
		content := g.header(m)
		if g.schemaFile != nil {
			files = append(files, generator.File{Path: schemaFileName(m), Content: g.schemaHeader(m)})
		}
		files = append(files, generator.File{Path: strings.ToLower(m.Name) + ".hpp", Content: content})
	}
...
type gen struct {
	...
	// schemaNeeded holds the emitted type names (message + reachable nested
	// struct/union types) for which a measure-phase schema descriptor is emitted
	// and installed via setSchema (generator#216). Pure-corelib-cpp only. Keyed by
	// the exported type name; populated once per message in Generate.
	schemaNeeded map[string]bool
	// schemaFile collects the measure-phase descriptors. They are corelib
	// machinery, not part of the user's message, so they are emitted into a
	// sibling header the message includes rather than into the message file
	// itself. nil until the first descriptor is written.
	schemaFile *hfile
	// schemaNeededOrder lists the same types in emission order, so the forward
	// declarations and the SchemaOf specialisations are deterministic.
	schemaNeededOrder []string
...
// schemaFileName is the sibling header holding a message's measure-phase
// descriptors — corelib machinery the message includes but does not contain.
func schemaFileName(m *ir.Message) string { return strings.ToLower(m.Name) + ".sofab.hpp" }

// schemaHeader wraps the collected descriptors in their own translation unit.
func (g *gen) schemaHeader(m *ir.Message) []byte {
	f := &hfile{}
	...
	f.blank()
	f.line("// Field bounds consulted while a field is measured for completeness,")
	f.line("// before its value is delivered.")
	f.blank()
	f.line("namespace %s {", g.ns)
	f.blank()
	// Declared here, defined by the message header that includes this file: the
	// SchemaOf specialisations below need the types declared, and this file must
	// also compile on its own.
	for _, n := range g.schemaNeededOrder {
		f.line("struct %s;", n)
	}
	f.blank()
	f.line("%s", strings.TrimRight(g.schemaFile.b.String(), "\n"))
	f.blank()
	f.line("} // namespace %s", g.ns)
	f.blank()
	f.line("// Bind each type to its descriptors: sofab::IStreamObject installs them,")
	f.line("// so the message itself carries no decoder plumbing.")
	for _, n := range g.schemaNeededOrder {
		f.line("template <> struct sofab::SchemaOf<%s::%s> {", g.ns, n)
		f.line("    static constexpr const sofab::schema::SeqNode *node = &%s::%s_schema;", g.ns, n)
		f.line("};")
	}
	return f.bytes()
}
...
func (g *gen) header(m *ir.Message) []byte {
	...
	// Measure-phase schema descriptors (generator#216, pure-corelib-cpp only): the
	// set of types whose over-count/over-maxlen/over-index bounds are enforced at
	// the deciding word before truncation (§5.2). Computed once, consulted in
	// emitStruct (descriptor emission) and decode/try_decode (setSchema install).
	g.schemaNeeded = g.computeSchemaNeeded(m)
	// Same order emitStruct will use (nested types first, message last), so the
	// forward declarations and SchemaOf specialisations are deterministic.
	g.schemaNeededOrder = nil
	for _, k := range order {
		if n := g.typeName(k); g.schemaNeeded[n] {
			g.schemaNeededOrder = append(g.schemaNeededOrder, n)
		}
	}
	if n := exported(m.Name); g.schemaNeeded[n] {
		g.schemaNeededOrder = append(g.schemaNeededOrder, n)
	}
	...
	f.line("#include %q", "sofab/sofab.hpp")
	if !g.clib && len(g.schemaNeeded) > 0 {
		// Declares the types, defines their bound descriptors and binds the two
		// via sofab::SchemaOf, so IStreamObject installs them itself and the
		// message below carries no decoder plumbing.
		f.line("#include %q", schemaFileName(m))
	}
	f.blank()
...
func (g *gen) emitStruct(f *hfile, name, summary string, fields []*ir.Field, isMessage bool) {
	// Measure-phase schema descriptor (generator#216): emitted just before the
	// struct so its decode()/try_decode() can install it, and after any nested
	// type it references (reachable() is post-order). Pure-corelib-cpp only.
	if !g.clib && g.schemaNeeded[name] {
		if g.schemaFile == nil {
			g.schemaFile = &hfile{}
		}
		g.emitSchemaDescriptor(g.schemaFile, name, fields)
	}
	emitStructDoc(f, summary)
	// sofab::Message is exactly the OStreamMessage + IStreamMessage pair (an empty
	// intermediate base, same layout). The c-cpp wrapper does not define it yet, so
	// that profile keeps spelling the pair out.
	base := "sofab::Message"
	if g.clib {
		base = "sofab::OStreamMessage, sofab::IStreamMessage"
	}
	f.line("struct %s : %s {", name, base)
...
// schemaBound is one FieldBound row: a field's schema bound at a sequence level.
...
// arrayElemWire maps a native array's element kind to the wire type its count
// header carries, so the measure walk matches the bound to the right field.
func arrayElemWire(elem ir.Kind) string {
	switch elem {
	case ir.KindFP32, ir.KindFP64:
		return "sofab::Wire::ArrayFixlen"
	case ir.KindI8, ir.KindI16, ir.KindI32, ir.KindI64, ir.KindEnum:
		return "sofab::Wire::ArraySigned"
	default: // u8..u64, bool, bitfield
		return "sofab::Wire::ArrayUnsigned"
	}
}

// schemaBounds returns the FieldBound rows for one sequence level: over-count
// native arrays, over-maxlen string/blob, over-index wrapper arrays, plus nested
// struct/union fields whose own subtree carries a bound (so the walk descends into
// them). A field with no bound is omitted -- an unlisted id carries no bound
// (S7.3). Element-level bounds of a wrapper array (a struct element's own fields)
...
func (g *gen) typeHasSchema(fields []*ir.Field) bool {
	return len(g.schemaBounds(fields)) > 0
}

// computeSchemaNeeded marks the message and every nested struct/union type
// transitively reachable through a struct/union FIELD that carries a bound. A type
// reached only as a wrapper-array element is not marked: its element-level bounds
// are not enforced (see schemaBounds), so no descriptor is emitted for it. Keyed
// by the exported type name, matching the name passed to emitStruct.
func (g *gen) computeSchemaNeeded(m *ir.Message) map[string]bool {
	needed := map[string]bool{}
	var visit func(name string, fields []*ir.Field)
	visit = func(name string, fields []*ir.Field) {
		if needed[name] || !g.typeHasSchema(fields) {
			return
		}
		needed[name] = true
		for _, fld := range fields {
			if fld.Kind == ir.KindStruct || fld.Kind == ir.KindUnion {
				k := fld.Ref.Key
				visit(g.typeName(k), g.schema.Named[k].Fields)
			}
		}
	}
	visit(exported(m.Name), m.Fields)
	return needed
}

// emitSchemaDescriptor writes the static FieldBound[] + SeqNode for one level.
// Named per emitted type so a parent references a child with &<child>_schema; the
