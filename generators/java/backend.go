...
func (g *gen) emitClass(f *jfile, name string, fields []*ir.Field, summary string, isMessage, isPublic bool) {
	...
	if isMessage {
		...
		f.line("    // The stream object is per-thread too, reset per call rather than")
		f.line("    // allocated: it is pure scratch that never outlives encode(), so a")
		f.line("    // fresh one per call is an allocation the caller cannot observe.")
		f.line("    private static final ThreadLocal<OStream> ENC_OS =")
		f.line("        ThreadLocal.withInitial(() -> new OStream(ENC_BUF.get()));")
		f.line("    public byte[] encode() {")
		f.line("        try {")
		f.line("            byte[] buf = ENC_BUF.get();")
		f.line("            OStream os = ENC_OS.get();")
		f.line("            os.reset(buf);")
		f.line("            marshal(os);")
		...
		// (generator#105).
		f.line("    // Per-thread decoder, reset per call rather than allocated (see ENC_OS).")
		f.line("    // Reset on entry, not exit, so a decode that threw cannot poison the")
		f.line("    // next one. Do not call decode()/tryDecode() reentrantly from a Visitor")
		f.line("    // callback on the same thread.")
		f.line("    private static final ThreadLocal<IStream> DEC_IS =")
		f.line("        ThreadLocal.withInitial(IStream::new);")
		f.line("    public static %s decode(byte[] data) {", name)
		f.line("        %s m = new %s();", name, name)
		f.line("        IStream is = DEC_IS.get();")
		f.line("        is.reset();")
		f.line("        try { is.feed(data, new %sVisitor(m)); }", name)
