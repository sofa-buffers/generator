...
func (g *gen) emitDecoder(f *jfile, name string) {
	...
	f.line("        // boundary.")
	f.line("        private DecodeStatus st = DecodeStatus.COMPLETE;")
	f.blank()
	f.line("        /**")
	...
	f.line("            return feed(chunk, 0, chunk.length);")
	f.line("        }")
	f.blank()
	...
	f.line("            try {")
	f.line("                return st = is.feed(chunk, off, len, v);")
	...
	f.line("                //")
	f.line("                // Anything else leaves the memory alone. ARGUMENT above all")
	f.line("                // says the mistake is in the CALL and not in the bytes; a")
	f.line("                // status is a verdict on the MESSAGE, so recording one for a")
	f.line("                // caller fault would report something about the wire that is")
	f.line("                // not true. This is the same three-way test IStream applies")
	f.line("                // in its own isTerminal().")
	f.line("                if (e.error() == SofabError.INVALID_MSG) {")
	f.line("                    st = DecodeStatus.INVALID;")
	f.line("                } else if (e.error() == SofabError.LIMIT_EXCEEDED) {")
	f.line("                    st = DecodeStatus.INCOMPLETE;")
	f.line("                }")
	f.line("                throw e;")
	f.line("            } catch (java.io.UncheckedIOException e) {")
	f.line("                // A Visitor cannot declare a checked exception, so a bound")
	f.line("                // this schema rejects, and a receiver limit this side")
	f.line("                // refuses, both arrive wrapped instead -- and so does an")
	f.line("                // ARGUMENT fault raised from inside a callback, which is why")
	f.line("                // this arm applies the same three-way test as the bare one")
	f.line("                // rather than sending everything unrecognized to INCOMPLETE.")
	f.line("                if (e.getCause() instanceof SofabException cause) {")
	f.line("                    if (cause.error() == SofabError.INVALID_MSG) {")
	f.line("                        st = DecodeStatus.INVALID;")
	f.line("                    } else if (cause.error() == SofabError.LIMIT_EXCEEDED) {")
	f.line("                        st = DecodeStatus.INCOMPLETE;")
	f.line("                    }")
	f.line("                }")
	f.line("                throw e;")
	...
	f.blank()
	f.line("        /**")
	f.line("         * The outcome for everything fed so far: what the last {@link")
	f.line("         * #feed(byte[])} returned, remembered here. The stream itself answers")
	f.line("         * only through that return value.")
	f.line("         */")
	f.line("        public DecodeStatus status() { return st; }")
	f.blank()
