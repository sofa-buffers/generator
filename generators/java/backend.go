...
func (g *gen) emitDecoder(f *jfile, name string) {
	...
	f.line("                // refuses, both arrive wrapped instead. Only malformed bytes")
	f.line("                // make the message INVALID; a limit is this side's policy, so")
	f.line("                // it leaves the message unfinished rather than wrong.")
	f.line("                if (e.getCause() instanceof SofabException cause")
	f.line("                        && cause.error() == SofabError.INVALID_MSG) {")
	f.line("                    st = DecodeStatus.INVALID;")
	f.line("                } else {")
