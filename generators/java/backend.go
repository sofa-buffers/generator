...
func (g *gen) emitDecoder(f *jfile, name string) {
	...
	// call. IStream.feed catches BOTH carriers for exactly that -- the bare
	// SofabException and the UncheckedIOException a Visitor's guard arrives in --
	// so the two arms that used to live here could only restate what it already
	// held, and flattened LIMIT_EXCEEDED into an INCOMPLETE that says something
	// untrue about the wire. finish asks the stream instead; see below.
	f.line("        public DecodeStatus feed(byte[] chunk, int off, int len) throws SofabException {")
	f.line("            return is.feed(chunk, off, len, v);")
	f.line("        }")
	f.blank()
	...
	f.line("         *")
	f.line("         * <p>Ending mid-field is an {@code IllegalStateException} and not a")
	...
	f.line("         *")
	f.line("         * <p>A stream that was REFUSED is the other case, and it keeps the")
	f.line("         * refusal's own code: the rejection is terminal, so asking the stream")
	f.line("         * again re-throws {@code INVALID_MSG} or {@code LIMIT_EXCEEDED} as")
	f.line("         * itself. A decoder that rejected a message cannot hand one back.")
	f.line("         *")
	f.line("         * @throws IllegalStateException the message ended inside a field or an open sequence.")
	f.line("         * @throws SofabException the stream had already refused these bytes; terminal.")
	f.line("         */")
	f.line("        public %s finish() throws SofabException {", name)
	// One call answers both halves. If the stream refused, its terminal guard runs
	// before a byte is looked at and re-throws that refusal's own code -- so a
	// decoder that was refused cannot hand back a message, and the caller sees
	// LIMIT_EXCEEDED as itself. If it did not, the return value is the outcome for
	// everything fed so far, computed from the stream's own state at this field
	// boundary; an empty chunk moves that state nowhere.
	...
	f.line("            DecodeStatus st = is.feed(new byte[0], 0, 0, v);")
