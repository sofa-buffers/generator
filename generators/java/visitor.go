...
//
// It returns the Bound to pass INTO the corelib call, and exactly one of the two
// bounds is ever live (ARCHITECTURE §9.5):
//
//   - schema `count: N` — Bound.schema(N), and a breach is INVALID_MSG (issue
//     #142, MESSAGE_SPEC §7.1): the bytes contradict the schema both peers agreed
//     on. A `count` is a CAPACITY, so the corelib refuses `id >= N`.
//   - no count — the receiver cap, Bound.receiver(MAX_DYN_ARRAY_COUNT), and a
//     breach is LIMIT_EXCEEDED (issue #387): the bytes are well-formed and the
//     same message decodes under a looser cap, so folding this into INVALID is
//     forbidden by §6.2.1.
//
// Either way the comparison is the corelib's and nothing is emitted in front of
// the call: the verdict travels with the number inside the Bound, so there is one
// implementation of each rule and never both on one field. Every wrapper-array
// placement goes through the corelib — Seq.placeElem for a string or blob
// element, Seq.reserveElem for a struct, union or nested-array element,
// Seq.reserveRow and the primitive reserveRow* factories for a matrix row — and
// each compares the bound it is handed BEFORE it grows anything (CORELIB_PLAN §7.2
// item 8). The sites with no placement to ride — the length word of a string or
// blob element, and a native row's header ahead of its element count — name the
// same Bound on Seq.checkIndex (see fixlenBeginArms and emitArrayBegin).
//
// Both are static final constants of the visitor (schemaCountConst, and the
// CAP_DYN_ARRAY_COUNT emitted beside MAX_DYN_ARRAY_COUNT), built once rather than
// per call, so the JIT folds each into the comparison it guards.
//
...
func (g *gen) indexBound(cap int64) string {
	if cap >= 0 {
		return schemaCountConst(cap)
	}
	if !g.limArr {
		panic("java: dynamic wrapper array with no array cap -- every target has a finite default (§9.5)")
	}
	return "CAP_DYN_ARRAY_COUNT"
}

// schemaCountConst names the visitor's `Bound.schema(n)` constant for a wrapper
// array whose schema declares `count: n`. One per distinct count, shared by every
// array that declares it (see schemaCounts).
func schemaCountConst(n int64) string {
	return fmt.Sprintf("SCHEMA_COUNT_%d", n)
}

// schemaCounts returns, ascending and without repeats, the `count` of every
// wrapper array in this message the schema bounds — exactly the set of
// schemaCountConst constants indexBound will name, because every non-normal frame
// is placed through one Seq call that takes its index bound. The twin of
// needsArrayCapBound for the schema half.
func schemaCounts(fs []frame) []int64 {
	seen := map[int64]bool{}
	var out []int64
	for _, fr := range fs {
		if fr.kind != fkNormal && fr.cap >= 0 && !seen[fr.cap] {
			seen[fr.cap] = true
			out = append(out, fr.cap)
		}
	}
...
// least one schema-unbounded field of that kind — an unbounded native array
...
// never masked to the width, never kept. An `enum` and a `bitfield` bind the
// width their declaration IMPLIES (§1) — the smallest signed type holding every
// constant, the smallest unsigned type holding the highest `pos` — and
...
//
// Storage is still never the bound. Java holds both kinds in a `long` — §1
// leaves the carrier to the target — so the comparison runs on the RAW
// accumulator, ahead of any narrowing, and a field whose declared positions are
// 0..3 does not become 0..2^63 valid because of it. §1's fourth consequence
...
func declaredWidthCond(k ir.Kind, ref *ir.TypeRef) string {
	...
	case ir.KindBitfield:
		...
		// 2^63 arrives with its sign bit set, and one mask on the raw bits
		// refuses both an over-width value and that sign-bit case in a single
		// operation. widthThrow's doc comment covers the same hazard.
...
func (g *gen) emitStringCb(f *jfile, fs []frame, limStr bool) {
	...
	// The receiver cap rides ON the accumulator call (§6.2.1); where it has to be
	// resolved per destination it rides on the destination gate below, which
	// already dispatches on exactly that.
	decl, capArg := payloadCap(dests, "CAP_DYN_STRING_LEN", limStr)
	capConst := ""
	if decl {
		emitCapDecl(f, "CAP_DYN_STRING_LEN")
	...
	// before it does either; null means more chunks are still to come.
	f.line("        String _s = acc.string(total, offset, data, chunkOffset, chunkLength, %s);", capArg)
	...
	for _, fr := range fs {
		if fr.kind == fkSeqLeaf && fr.elemKind == ir.KindString {
			...
			// takes the array's index bound as its last argument, so both verdicts --
			// INVALID past a schema count, LIMIT_EXCEEDED past the cap -- are the
			// corelib's and never appear here.
			f.line("        case %d: Seq.placeElem(%s, id, \"\", _s, %s); break;", fr.idx, fr.listExpr, g.indexBound(fr.cap))
			continue
		}
		if fr.kind != fkNormal {
			continue
...
func (g *gen) emitBlobCb(f *jfile, fs []frame, limBlob bool) {
	...
	// The receiver cap rides ON the accumulator call (§6.2.1), and where it has to
	// be resolved per destination it rides on the destination gate below — the
	// same shape emitStringCb takes, which is only available here because that
	// gate now covers blobs too.
	decl, capArg := payloadCap(dests, "CAP_DYN_BLOB_LEN", limBlob)
	capConst := ""
	if decl {
		emitCapDecl(f, "CAP_DYN_BLOB_LEN")
	...
	// are still to come.
	f.line("        byte[] _b = acc.blob(total, offset, data, chunkOffset, chunkLength, %s);", capArg)
	...
	for _, fr := range fs {
		if fr.kind == fkSeqLeaf && fr.elemKind == ir.KindBlob {
			...
			f.line("        case %d: Seq.placeElem(%s, id, Seq.EMPTY_BYTES, _b, %s); break;", fr.idx, fr.listExpr, g.indexBound(fr.cap))
			continue
		}
		if fr.kind != fkNormal {
			continue
...
//
// `dynIds` / `dyn` split those destinations by whether the SCHEMA bounds them.
// The receiver cap governs only the unbounded ones (CORELIB_PLAN §6.2.1: it
// "MUST NOT be applied to a field the schema already bounds"), and the cap is
// now an argument to the corelib call rather than a guard in front of it, so the
// split has to be resolved per destination before that call is made.
type destFrame struct {
	idx    int
	ids    []int64 // every destination id in this frame
	dynIds []int64 // the subset the schema leaves unbounded (no maxlen)
	dyn    bool    // wrapper-sequence row whose element declares no maxlen
}

// allDyn reports whether every destination in this frame is schema-unbounded.
func (d destFrame) allDyn() bool {
	if len(d.ids) == 0 {
		return d.dyn
	}
	return len(d.dynIds) == len(d.ids)
}
...
func kindDests(fs []frame, kind ir.Kind) []destFrame {
	...
	for _, fr := range fs {
		if fr.kind == fkSeqLeaf && fr.elemKind == kind {
			dests = append(dests, destFrame{idx: fr.idx, dyn: !fr.elemMaxHas})
			continue
		}
		if fr.kind != fkNormal {
			continue
		...
		for _, fld := range fr.fields {
			if fld.Kind == kind {
				...
				if !fld.HasMaxlen {
					dyn = append(dyn, fld.ID)
				}
			}
		}
		if len(ids) > 0 {
			dests = append(dests, destFrame{idx: fr.idx, ids: ids, dynIds: dyn})
		}
	}
	return dests
}
...
//
// `capConst` non-empty makes this switch carry the receiver cap as well: the
// destination it already resolves is exactly what the cap depends on (§6.2.1 —
// the cap governs a schema-unbounded field and nothing else), so an arm falling
// through to a dynamic destination sets `_lim` to the constant and a
// schema-bounded one leaves it at its Bound.SCHEMA_BOUNDED initializer. That
// costs one store on a dispatch already here, which is why the cap is not a
// switch of its own — and because the gate has already returned for everything
// that is not a destination, an undeclared id and a §7.3 contradiction reach the
// accumulator with no cap by construction (§6.2.1: a skipped field is never
// capped).
func (g *gen) emitDestGuard(f *jfile, fs []frame, dests []destFrame, capConst string) {
	...
	for _, d := range dests {
		if len(d.ids) == 0 {
			if d.dyn {
				f.line("        case %d: %sbreak;", d.idx, set)
			} else {
				f.line("        case %d: break;", d.idx)
			}
			continue
		}
		// Two label groups per frame at most: the ids the cap governs, then the
		// ids a schema maxlen governs instead. With no cap in play (capConst empty)
		// they collapse into the single fall-through arm this used to emit.
		var arms []string
		if capConst != "" && len(d.dynIds) > 0 {
			arms = append(arms, caseLabels(d.dynIds)+" "+set+"break;")
		}
		if rest := boundedIDs(d, capConst != ""); len(rest) > 0 {
			arms = append(arms, caseLabels(rest)+" break;")
		}
		f.line("        case %d: switch (id) { %s default: return; } break;", d.idx, strings.Join(arms, " "))
	}
	f.line("        default: return;")
	f.line("        }")
}

// caseLabels renders ids as consecutive `case N:` switch labels.
func caseLabels(ids []int64) string {
	var out []string
	for _, id := range ids {
		out = append(out, fmt.Sprintf("case %d:", id))
	}
	return strings.Join(out, " ")
}

// boundedIDs returns the frame's destination ids minus the schema-unbounded ones
// when the two are being split, and all of them otherwise.
func boundedIDs(d destFrame, split bool) []int64 {
	if !split {
		return d.ids
	}
	dyn := map[int64]bool{}
	for _, id := range d.dynIds {
		dyn[id] = true
	}
	var out []int64
...
// payloadCap resolves the receiver cap for one payload callback: what to hand
// acc.string / acc.blob as its `max` argument, and whether a `_lim` local has to
// be declared and filled in per destination first.
//
// Since corelib-java 0.12.0 the comparison lives in PayloadAcc, against the
// announced `total` at the top of the method — before the whole-in-one-chunk
// path materializes anything and before the split path takes a byte into the
// accumulator (CORELIB_PLAN §6.2.1: "A corelib MAY take a limit as an argument
// and perform the check itself, and a port that does is conformant"). So this
// layer states the NUMBER and nothing else; there is no guard in front of the
// call, because §6.2.1 admits one implementation of the rule and that is it.
//
...
//   - mixed — `_lim`, initialized to Bound.SCHEMA_BOUNDED and raised to the cap
//     on the destinations the schema leaves open.
//
// The second shape has a PRECONDITION: it is sound only because both callers
// emit the destination gate ahead of the call, so everything that is not a
// destination has already returned and the literal cannot reach it. Blob had no
// gate until generator#436 and could not take it — a bare constant there would
// have capped an undeclared id and a §7.3 contradiction, which §6.2.1's "a
// skipped field is never capped" forbids. A payload callback added without a
// gate must not use this.
func payloadCap(dests []destFrame, constName string, lim bool) (decl bool, arg string) {
	if !lim {
		return false, "Bound.SCHEMA_BOUNDED"
	}
	all := true
	for _, d := range dests {
		if !d.allDyn() {
...
// emitCapDecl writes the `_lim` local the mixed shape needs. It starts at
// Bound.SCHEMA_BOUNDED so that anything the arms below do not name — a
// schema-bounded destination, or (in blob(), which has no destination gate) a
// payload this message never reads — carries no receiver cap at all.
func emitCapDecl(f *jfile, constName string) {
	f.line("        // The receiver cap for the destination this payload is headed for,")
	f.line("        // handed to the corelib accumulator, which compares it against the")
	f.line("        // announced length before it buffers or materializes a byte (S6.2.1).")
	f.line("        // It stays SCHEMA_BOUNDED where a schema maxlen governs instead, and")
	f.line("        // for a payload this message does not read at all -- a skipped field")
	f.line("        // is never capped.")
	f.line("        Bound _lim = Bound.SCHEMA_BOUNDED;   // raised to %s where the schema declares no maxlen", constName)
}
...
func (g *gen) emitFixlenBegin(f *jfile, fs []frame) {
	...
	blob := g.fixlenBeginArms(fs, ir.KindBlob, "blob length", capConstFor(limBlob, "CAP_DYN_BLOB_LEN"))
...
// capConstFor names the Bound constant that carries a kind's receiver cap, or ""
// where this project configured none for it -- so a caller can ask for the cap
// arms without also re-deriving whether they exist.
...
// implementations of it -- what §6.2.1 forbids ("one implementation, wherever it
// runs") is a second copy in another layer, and this layer states the number and
// makes the call, as it already did. corelib-go's collectors have the same shape
// for the same reason.
func capCheck(kind ir.Kind, capConst string) string {
	if kind == ir.KindBlob {
		return fmt.Sprintf("PayloadAcc.checkBlobLength(total, %s);", capConst)
	}
	return fmt.Sprintf("PayloadAcc.checkStringLength(total, %s);", capConst)
}
...
// which is compared by the corelib call. Never both on one field, and never one
// answering in the other's category.
//
// capConst is "" for a kind this project configured no cap for; those
// destinations get no arm at all rather than an arm that caps nothing.
func (g *gen) fixlenBeginArms(fs []frame, kind ir.Kind, noun, capConst string) []string {
	var arms []string
	for _, fr := range fs {
		...
		if fr.kind == fkSeqLeaf && fr.elemKind == kind && (fr.cap >= 0 || fr.emax >= 0 || elemCapped) {
			...
			// bound is named on Seq.checkIndex, the same comparison Seq.placeElem
			// makes a moment later against the same folded constant, so the two
			// sites cannot drift apart.
			body := "Seq.checkIndex(id, " + g.indexBound(fr.cap) + "); "
			switch {
			case fr.emax >= 0:
				body += fmt.Sprintf("if (total > %d) %s ", fr.emax, maxlenThrow(locName(fr.loc)+" element", noun, fr.emax))
			case elemCapped:
				body += capCheck(kind, capConst) + " "
			}
		...
		for _, fld := range fr.fields {
			...
			case capConst != "":
				inner = append(inner, fmt.Sprintf("case %d: %s break;", fld.ID, capCheck(kind, capConst)))
			}
		}
		if len(inner) > 0 {
...
func (g *gen) emitVisitor(f *jfile, name string, fields []*ir.Field) {
	...
	if limArr || limStr || limBlob {
		...
		// Beside each number, the sofab.Bound that carries it INTO the corelib
		// call that compares it (§6.2.1). Bound.receiver is the only way to make
		// one, so every number reaching a cap comparison is a receiver cap the
		// deployment configured -- there is no sentinel a forgotten argument and
		// "the schema bounds this field" could share. Built once into a static
		...
		if limStr {
			...
			f.line("    private static final Bound CAP_DYN_STRING_LEN = Bound.receiver(MAX_DYN_STRING_LEN);")
		}
		if limBlob {
			f.line("    static final long MAX_DYN_BLOB_LEN = %dL;", g.limits.blobLen)
			f.line("    private static final Bound CAP_DYN_BLOB_LEN = Bound.receiver(MAX_DYN_BLOB_LEN);")
		}
	}
	// The schema half of the same argument: a wrapper array's declared `count`,
	// handed to the Seq call that places its elements, which compares the index
	// against it and answers INVALID_MSG past it (MESSAGE_SPEC §7.1). One constant
	// per distinct count, built once like the caps above.
	if counts := schemaCounts(fs); len(counts) > 0 {
		f.line("    // Schema capacities of this message's wrapper arrays: an element index at")
		f.line("    // or past its array's count is INVALID_MSG, compared by the corelib call")
		f.line("    // that places the element, before the list grows.")
	...
	for i := range fs {
		...
		if fr.kind == fkNativeMat {
			...
			// differently is a conformance diff waiting to happen. A SCHEMA count
			// is therefore named on Seq.checkIndex ahead of the count guard; a
			// receiver cap is compared by the reserveRow call alone (§6.2.1), before
			// it allocates the row or grows the outer list. The reserveRow call is
			// handed the same Bound either way and re-runs the comparison against
			// the same folded constant.
			idxCap := g.indexBound(fr.cap)
			idxCheck := ""
			if fr.cap >= 0 {
				idxCheck = "Seq.checkIndex(id, " + idxCap + "); "
			}
			arm := kindGuard + idxCheck + guard + armFill(fs, fr, nil)
			...
			if primitiveArrayElem(fr.innerElem) {
				...
				f.line("        case %d: %s%s = Seq.%s(%s, id, count, %s); %s = id; break;",
					fr.idx, arm, rowCursor(base), reserveRowFn(base), fr.listExpr, idxCap, elemIdxVar(fr.loc))
				continue
			}
			f.line("        case %d: %sSeq.reserveRow(%s, id, %s); %s = id; break;", fr.idx, arm, fr.listExpr, idxCap, elemIdxVar(fr.loc))
			continue
		}
		if fr.kind != fkNormal {
			continue
	...
	for _, fr := range fs {
		...
		case fkSeqObj:
			...
			f.line("        case %d: Seq.reserveElem(%s, id, %s::new, %s); %s = id; cur = %d; break;",
				fr.idx, fr.listExpr, fr.elemType, g.indexBound(fr.cap), elemIdxVar(fr.loc), locIndex(fs, fr.childLoc))
		case fkSeqMat:
			...
			// Placed by the corelib, so the index bound -- schema count or receiver
			// cap -- is that call's argument and nothing guards in front of it.
			f.line("        case %d: Seq.reserveRow(%s, id, %s); %s = id; cur = %d; break;", fr.idx, fr.listExpr, g.indexBound(fr.cap), elemIdxVar(fr.loc), locIndex(fs, fr.childLoc))
		case fkNormal:
			var arms []string
			for _, fld := range fr.fields {
				switch {
