	...
	for i, v := range vals {
		...
		case ir.KindBitfield:
			...
			if bits, err := strconv.ParseUint(scalarLit(v), 10, 64); err == nil {
				...
				// checkArrayElem grew a "bitfield" arm (generator#482), and every
				// spelling that survives it is a non-negative decimal ParseUint
				// accepts, so the branch above always wins. Kept correct so that a
				// future change to the validator or to element lowering cannot
				// silently turn -1 into 0 — parsing a stray spelling as a mask
				// would change the VALUE, so the text is carried through unaltered
				// and the C compiler judges it.
				e = scalarLit(v)
			}
