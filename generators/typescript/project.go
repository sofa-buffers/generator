...
func (g *gen) harness(s *ir.Schema) []byte {
	...
	f.blank()
	// decode()'s result carries marshal() too, so `recode` can re-encode the
	// decoded object without a JSON round-trip (issue #235).
	f.line("const MESSAGES: Record<string, { fromJSON(d: Record<string, unknown>): { marshal(os: import(%q).OStream): void; toJSON(): Record<string, unknown> }; decode(b: Uint8Array): { marshal(os: import(%q).OStream): void; toJSON(): Record<string, unknown> } }> = {", corelibPkg, corelibPkg)
	...
	// Wire -> object -> wire with no JSON in between: the byte-exact re-encode
	// path an fp32 signaling NaN must survive (issue #235 / MESSAGE_SPEC §4.6).
	// Going through JSON would launder the value through a decimal literal and
	// hide exactly the bits under test.
	f.line("    const obj = cls.decode(new Uint8Array(input));")
	f.line("    const os = new (await import(%q)).OStream();", corelibPkg)
	f.line("    obj.marshal(os);")
	f.line("    process.stdout.write(os.bytes());")
