...
func (g *gen) harness(s *ir.Schema) []byte {
	...
	f.blank()
	// decode() is typed with serialize() as well as toJSON(): the `recode` mode below
	// re-encodes a decoded object without going through JSON, which is the only way
	// to observe wire detail a JSON value cannot hold (an fp32 signaling NaN's
	// payload bits, say).
	f.line("const MESSAGES: Record<string, { fromJSON(d: Record<string, unknown>): { serialize(os: import(%q).OStream): void; toJSON(): Record<string, unknown> }; decode(b: Uint8Array): { serialize(os: import(%q).OStream): void; toJSON(): Record<string, unknown> } }> = {", corelibPkg, corelibPkg)
	...
	for _, m := range s.Messages {
		...
		f.line("    const setup = new OStream(); obj.serialize(setup);")
		...
		f.line("      ? () => { const os = new OStream(); obj.serialize(os); sink ^= os.bytes().length; }")
