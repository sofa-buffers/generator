...
// The project's non-TypeScript files are written the way prettier writes them
// -- expanded objects, two-space indent, trailing newline -- so that a user
// running `prettier --check .` over their tree sees nothing here, whatever the
// --format switch was set to. Unlike the .ts files, this needs no tool at
// generation time: prettier's JSON output has no width-driven choices to
// reproduce (generators/typescript/format.go).
const packageJSON = `{
...
func (g *gen) harness(s *ir.Schema) []byte {
	...
	// A schema of $defs alone has no message: nothing below would name M, the
	// warmup count or the bench inputs, and each would be an unused declaration.
	...
	// `feed` and `finish` are the whole decode surface: the wrapper keeps no
	// remembered status to name here, because a refusal is terminal in the corelib
	...
	f.line("    } catch (e) {")
	// What finish answers AFTER the refusal is printed, not just the throw. A
	// refusal is terminal: the corelib latched it and re-throws the very code it
	// was refused with, so a finish here must refuse too, under that same code.
	// That is the whole of what generated code contributes now (generator#541),
	// and a reject vector exits 1 either way, so naming finish's answer is the
	// only way a suite sees it.
	//
	// `RETURNED` is the failure: finish handed back a message from a decoder that
	// had refused one. It cannot be a leftover from an earlier call the way the
	// old remembered status could (#528) -- nothing but this finish produces it.
	// `.code` is the machine-readable string the corelib puts on a SofabError --
	// the same one the `status` mode below matches on -- so no import is needed to
	// name it.
	f.line("      let fin;")
	f.line("      try { dec.finish(); fin = \"RETURNED\"; }")
	f.line("      catch (fe) { fin = (fe as { code?: string })?.code ?? String(fe); }")
