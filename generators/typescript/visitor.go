...
func (g *gen) emitDecoderClass(f *tsfile, name string) {
	...
	f.line("  // that return value, and offers no accessor to ask a second time, so the")
	f.line("  // caller is the one that remembers -- and this decoder is the caller.")
	f.line("  // `Complete` before the first feed: an all-default message is zero bytes,")
	f.line("  // so a stream that has been fed nothing ended on a field boundary.")
	f.line("  private st: DecodeStatus = DecodeStatus.Complete;")
	f.blank()
	...
	f.line("    try {")
	f.line("      return (this.st = this.is.feed(chunk));")
	f.line("    } catch (e) {")
	f.line("      // A refusal is terminal and never comes back as a status, so record")
	f.line("      // what it means for the stream before rethrowing. Malformed bytes make")
	f.line("      // the message Invalid; a receiver limit is this side's policy, so it")
	f.line("      // leaves the message unfinished rather than wrong.")
	f.line("      //")
	f.line("      // Anything else leaves the memory alone: a TypeError out of a")
	f.line("      // callback, or a SofabError carrying Argument, says the mistake is in")
	f.line("      // the CALL and not in the bytes. A status is a verdict on the")
	f.line("      // MESSAGE, so recording one for a fault that is not a wire event")
	f.line("      // would report something about the wire that is not true.")
	f.line("      if (e instanceof SofabError) {")
	f.line("        if (e.code === SofabErrorCode.InvalidMsg) {")
	f.line("          this.st = DecodeStatus.Invalid;")
	f.line("        } else if (")
	f.line("          e.code === SofabErrorCode.LimitExceeded ||")
	f.line("          e.code === SofabErrorCode.Incomplete")
	f.line("        ) {")
	f.line("          this.st = DecodeStatus.Incomplete;")
	f.line("        }")
	f.line("      }")
	f.line("      throw e;")
	f.line("    }")
	...
	f.blank()
	f.line("  /**")
	f.line("   * The outcome for everything fed so far: what the last `feed` returned,")
	f.line("   * remembered here. The stream itself answers only through that return")
	f.line("   * value, so this is the wrapper's memory of it, not a second question")
	f.line("   * put to the stream.")
	f.line("   */")
	f.line("  get status(): DecodeStatus { return this.st; }")
	f.blank()
	...
	f.line("    if (this.st !== DecodeStatus.Complete) {")
