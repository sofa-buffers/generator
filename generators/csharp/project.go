...
func (g *gen) harness(s *ir.Schema) []byte {
	...
	for _, m := range s.Messages {
		...
		// What Finish answers AFTER the refusal is printed, not just the message.
		// A refusal is terminal: the corelib latched it and re-throws the very
		// code it was refused with, so a Finish here must refuse too, under that
		// same code. That is the whole of what generated code contributes now
		// (generator#541), and a reject vector exits non-zero either way -- so
		// naming Finish's answer is the only way a suite can see it at all.
		//
		// `RETURNED` is the failure: it means Finish handed back a message from a
		// decoder that had refused one. It cannot be a leftover from an earlier
		// call the way the old remembered status could (#528), because nothing
		// but this Finish produces it.
		f.line("                    string fin;")
		f.line("                    try { dec.Finish(); fin = \"RETURNED\"; }")
		f.line("                    catch (SofabException fe) { fin = fe.Error.ToString(); }")
		f.line("                    catch (Exception fe) { fin = fe.GetType().Name; }")
		f.line("                    Console.Error.WriteLine(")
		f.line("                        $\"decode error: {e.Message} [finish={fin}]\");")
		f.line("                    return 1;")
		f.line("                }")
		f.line("                var json = JsonSerializer.SerializeToUtf8Bytes(obj, Opts);")
		f.line("                stdout.Write(json, 0, json.Length); stdout.WriteByte((byte)'\\n');")
