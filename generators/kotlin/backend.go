...
//     integers at.
//   - the `@file:Suppress` names are IDE inspections only; none of them is a
//     compiler diagnostic, so an allWarningsAsErrors build still sees every
//     compiler warning.
//
// DEPRECATION is not suppressed file-wide: that would also hide a deprecated
// corelib API the generated code calls. The declarations that must touch a
// `deprecated:` field carry it themselves (deprecationSuppress).
func (g *gen) header(f *kfile) {
	...
	f.line("@file:Suppress(\"RedundantVisibilityModifier\", \"unused\", \"RemoveRedundantCallsOfConversionMethods\", \"KotlinConstantConditions\")")
	f.blank()
...
// deprecationSuppress is the annotation a generated declaration carries when it
// must read or write a `deprecated:` field: that is the point of the field still
// being on the wire, and the @Deprecated marker is for the CALLER (ARCHITECTURE
// §8). It is emitted only where such a field is touched, so a deprecation
// warning anywhere else -- a deprecated corelib API, say -- still fails an
// allWarningsAsErrors build.
const deprecationSuppress = "@Suppress(\"DEPRECATION\") // generated code must still read and write deprecated fields"

// anyDeprecated reports whether any of fields is marked deprecated.
func anyDeprecated(fields []*ir.Field) bool {
...
func (g *gen) emitDecoder(f *kfile, name string) {
	...
	// call -- and it keeps a third latch of its own for a destination it
	// refused, which this layer never mapped at all. Recording a second copy
	// here bought nothing the stream did not already hold, and flattened
	// LIMIT_EXCEEDED into an INCOMPLETE that says something untrue about the
	// wire. finish asks the stream instead; see below.
	f.line("        public fun feed(chunk: ByteArray): DecodeStatus = feed(chunk, 0, chunk.size)")
	f.blank()
	...
	f.line("         * @throws SofabException the stream had already refused these bytes;")
	f.line("         *   the rejection is terminal, so asking it again re-throws the code")
	f.line("         *   it was refused with. A decoder that rejected a message cannot")
	f.line("         *   hand one back.")
	f.line("         */")
	...
	f.line("            val st = ist.feed(ByteArray(0), 0, 0, v)")
	f.line("            check(st == DecodeStatus.COMPLETE) { \"%s: stream ended mid-field (\" + st + \")\" }", name)
	f.line("            return m")
	f.line("        }")
	f.line("    }")
