...
func (g *gen) emitDecoder(f *kfile, name string) {
	...
	f.line("        // COMPLETE before the first feed: an all-default message is zero bytes, so")
	f.line("        // a stream that has been fed nothing ended on a field boundary.")
	f.line("        private var st: DecodeStatus = DecodeStatus.COMPLETE")
	f.blank()
	f.line("        /**")
	...
	f.line("         */")
	// Delegates rather than calling the corelib's own one-argument overload: the
	// remembered value then has exactly ONE write site, so the two overloads
	// cannot drift apart. Same shape as the Java backend.
	...
	f.line("            try {")
	f.line("                st = ist.feed(chunk, off, len, v)")
	f.line("            } catch (e: SofabException) {")
	...
	f.line("                // Malformed bytes make the message INVALID; a receiver limit is")
	f.line("                // this side's policy, so it leaves the message unfinished")
	f.line("                // rather than wrong -- the two are never folded together.")
	f.line("                //")
	f.line("                // Anything else leaves the memory alone. ARGUMENT says the mistake")
	f.line("                // is in the CALL and not in the bytes, and a status is a verdict on")
	f.line("                // the MESSAGE, so recording one for a caller fault would report")
	f.line("                // something about the wire that is not true. This is the same")
	f.line("                // three-way test IStream applies to its own latches.")
	f.line("                when (e.error) {")
	f.line("                    SofabError.INVALID_MSG -> st = DecodeStatus.INVALID")
	f.line("                    SofabError.LIMIT_EXCEEDED -> st = DecodeStatus.INCOMPLETE")
	f.line("                    else -> Unit")
	f.line("                }")
	f.line("                throw e")
	f.line("            }")
	...
	f.blank()
	f.line("        /**")
	f.line("         * The outcome for everything fed so far, without feeding more: what the")
	f.line("         * last [feed] returned, remembered here. The stream itself answers only")
	f.line("         * through that return value.")
	f.line("         */")
	f.line("        public val status: DecodeStatus get() = st")
	f.blank()
	...
	// Reads the remembered value: finish() feeds nothing, so there is no return
	// value to take, and the stream will not answer again.
	f.line("            check(st == DecodeStatus.COMPLETE) { \"%s: stream ended mid-field (\" + st + \")\" }", name)
	f.line("            return m")
	f.line("        }")
	f.line("    }")
