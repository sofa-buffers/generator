...
func (g *gen) sbufSupport(refs string) []byte {
	// What the rest of the package actually names. Asking the emitted source is
	// exact and cannot drift from the call sites, because it reads them: a member
	// is kept iff something calls it. The `Sbuf.` prefix keeps the lookups apart
	// -- `Sbuf.EMPTY_SHORT` does not match inside `Sbuf.EMPTY_USHORT`, and
	// `Sbuf.placeRowLong` not inside `Sbuf.placeRowList`.
	used := func(name string) bool { return strings.Contains(refs, "Sbuf."+name) }
	// A kept placeRow fills its gaps with the matching EMPTY_, so it pulls that
	// constant in even where no field initializer names it.
	keepEmpty := func(t string) bool {
		return used("EMPTY_"+strings.ToUpper(baseSuffix(t))) || used("placeRow"+baseSuffix(t))
	}
	// A section's doc block is worth emitting only if a member under it survives.
	anyOf := func(prefix string) bool {
		for _, t := range primBaseOrder {
			if used(prefix + baseSuffix(t)) {
				return true
			}
		}
		return false
	}
	anyEnsure, anyPlace := anyOf("ensureCap"), anyOf("placeRow")
	anyEmpty := false
	for _, t := range primBaseOrder {
		anyEmpty = anyEmpty || keepEmpty(t)
	}
	// Nothing calls into Sbuf at all -- a schema of plain scalars, say. Emitting
	// an empty object would be the very artifact this file exists to avoid, so
	// the caller drops the file instead.
	if !anyEmpty && !anyEnsure && !anyPlace &&
		!used("Acc") && !used("placeRowList") && !used("boolBytes") {
		return nil
	}
	f := &kfile{}
	g.header(f)
	...
	if anyEnsure {
		...
		f.line("     */")
		for _, t := range primBaseOrder {
			if !used("ensureCap" + baseSuffix(t)) {
				continue
			}
