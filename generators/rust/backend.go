...
func (*Backend) Generate(s *ir.Schema, cfg map[string]any) ([]generator.File, error) {
	g := &gen{schema: s, banner: cfgString(cfg, "tool_banner", "sofabgen"), omit: cfgBool(cfg, "omit_defaults"), corelib: cfgString(cfg, "corelib", "rs-no-std")}
...
type gen struct {
	...
	banner string
	omit   bool
	// corelib selects the Rust corelib: "rs-no-std" (default, corelib-rs-no-std —
	// #![no_std], heap-free, Cargo feature flags to shrink the binary) or "rs"
	// (corelib-rs — std, high-throughput, every wire type always compiled in, no
	// feature flags and no require! capability guard).
	corelib string
}

// std reports whether the std corelib-rs is selected (vs corelib-rs-no-std).
func (g *gen) std() bool { return g.corelib == "rs" }
