...
// finite default that the max_dyn_* key only overrides (§9.5, generator#385) —
// so an entry is active exactly when the schema actually has an unbounded field
// of that kind; otherwise the cap would be inert and no plumbing is emitted.
// Resolution is skipped entirely on corelib-rs-no-std, which rejects an
// unbounded field at schema validation and has nothing for a cap to govern. The
