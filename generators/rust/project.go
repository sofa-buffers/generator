...
func (g *gen) emitBench(f *rfile, s *ir.Schema) {
	...
	// A schema without a message has no workload to decode, so the input goes
	// unread; the underscore says so instead of leaving an unused variable.
	input := "input"
	if len(s.Messages) == 0 {
		input = "_input"
	}
...
`
	...
	// third-party container crate to every generated project would be a cost paid
	// by projects that never asked for inline storage.
	heap := ""
	if g.staticStore {
...
func (g *gen) harness(s *ir.Schema) []byte {
	...
	f.line("    }")
	if !hasMsg {
		// Nothing to dispatch on: a match whose only arm is the wildcard is a
		// clippy::match_single_binding, so the fallback stands alone.
		f.line("    eprintln!(\"unknown message\");")
