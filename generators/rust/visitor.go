...
func (g *gen) emitVisitor(f *rfile, name string, fields []*ir.Field) {
	...
	f.line("    pub fn try_decode(data: &[u8]) -> Result<%s, DecodeError> {", name)
	f.line("        let mut m = %s::default();", name)
	f.line("        let overflow;")
	f.line("        let invalid;")
	if g.limits.any() {
	...
	f.line("        if invalid { return Err(DecodeError::Sofab(sofab::Error::InvalidMsg)); }")
	if g.limits.any() {
		f.line("        // A structural INVALID from feed still dominates: those bytes are")
		f.line("        // malformed regardless of what follows (S5.2.3).")
		f.line("        if let Err(sofab::Error::InvalidMsg) = fed { return Err(DecodeError::Sofab(sofab::Error::InvalidMsg)); }")
		...
		f.line("        if limited { return Err(DecodeError::Sofab(sofab::Error::LimitExceeded)); }")
	}
	f.line("        // Nothing refused above: now surface feed's own verdict. A structural")
	f.line("        // InvalidMsg rides `?`; Status::Incomplete does NOT -- the corelib leaves")
	f.line("        // the end-of-input judgement to the caller (S5.2.4) because only the")
	f.line("        // caller's framing knows whether more bytes come, and this entry point IS")
	f.line("        // that framing: it was handed the whole buffer, so an Incomplete here is")
	f.line("        // a truncated message and is rejected, not returned half-filled (S7).")
	f.line("        if fed? == sofab::Status::Incomplete { return Err(DecodeError::Incomplete); }")
	f.line("        // A fixed-capacity field overflowed during the fill:")
	f.line("        // report it rather than return a silently-truncated value.")
	f.line("        if overflow { return Err(DecodeError::Sofab(sofab::Error::BufferFull)); }")
	f.line("        Ok(m)")
	f.line("    }")
	f.blank()
	...
	f.line("    /// in, and it is a `Status`, not an error: `Status::Complete` means they")
	f.line("    /// ended on a clean field boundary (the message COULD end here),")
	f.line("    /// `Status::Incomplete` means they ended mid-field. Neither is a failure")
	f.line("    /// mid-stream, which is why both live in the success arm (S5.2.1). The")
	f.line("    /// caller's own framing -- a length prefix, a datagram boundary, a closed")
	f.line("    /// socket -- decides when to stop; `finish` then gives the verdict for the")
	f.line("    /// message as a whole, and is the only place a trailing `Status::Incomplete`")
	...
	f.line("        /// boundary, `Ok(Status::Incomplete)` if it ended mid-field -- see the")
	f.line("        /// type docs: neither answers whether the MESSAGE is done, only whether")
	f.line("        /// these bytes were. `Err` is a refusal, and it is terminal.")
	f.line("        pub fn feed(&mut self, chunk: &[u8]) -> Result<sofab::Status, sofab::Error> {")
	f.line("            let fed = {")
	...
	f.line("        pub fn finish(mut self) -> Result<%s, DecodeError> {", name)
	f.line("            if self.inv { return Err(DecodeError::Sofab(sofab::Error::InvalidMsg)); }")
	if g.limits.any() {
		...
		f.line("            if self.lim { return Err(DecodeError::Sofab(sofab::Error::LimitExceeded)); }")
	}
	f.line("            // An empty chunk probes end-of-input without supplying any: Complete")
	f.line("            // only when nothing is half-read. The caller's framing has said the")
	f.line("            // input is over, so an Incomplete here is truncation -- rejected,")
	f.line("            // never handed back as a silently partial value (S7).")
	f.line("            if self.feed(&[])? == sofab::Status::Incomplete {")
	f.line("                return Err(DecodeError::Incomplete);")
	f.line("            }")
	f.line("            if self.err { return Err(DecodeError::Sofab(sofab::Error::BufferFull)); }")
	f.line("            Ok(self.m)")
	f.line("        }")
	f.line("    }")
	f.blank()
...
// (the frames differ, the arm does not), and the two array positions route the
// same conditions through arrayWidthGuard — so a value the schema does not
// declare gets one verdict wherever it lands (generator#515/#516).
...
// elemWidthCond is widthCond over an ARRAY ELEMENT, and it answers for the two
// kinds widthCond cannot: `enum` and `bitfield`, whose declaration lives on the
// NAMED TYPE rather than on the Kind. ir.NarrowRange takes a Kind alone, so it
// returns !ok for both and the element store used to be emitted bare — a wire
// element of 1000 was truncated by the `as` cast into `[232]` / `[-24]` and the
