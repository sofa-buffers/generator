		...
		if g.noStd {
			...
			for _, fr := range fs {
				if fr.kind == fkSeqArr && fr.elemKind == ir.KindBlob {
					f.line("            (_Loc::%s, _) => { %s if let Some(_e) = %s.get_mut(id as usize) { let _ = _e.extend_from_slice(_b); } }", fr.loc, g.seqElemGrow(fr.path), fr.path)
				}
				for _, fld := range fr.fields {
					if fld.Kind == ir.KindBlob {
						f.line("            (_Loc::%s, %d) => { %s.%s.clear(); let _ = %s.%s.extend_from_slice(_b); }", fr.loc, fld.ID, fr.path, rustIdent(fld.Name), fr.path, rustIdent(fld.Name))
					}
				}
			}
			f.line("            _ => {}")
