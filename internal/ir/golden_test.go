...
//
//	go run ./cmd/sofabgen --dump-ir --in examples/messages/example.yaml > internal/ir/testdata/example.ir.json
//
...
func TestExampleIRGolden(t *testing.T) {
	...
	if string(got) != string(want) {
		t.Fatalf("IR drift vs golden %s.\nRegenerate if intentional:\n  go run ./cmd/sofabgen --dump-ir --in examples/messages/example.yaml > %s\n--- got ---\n%s",
