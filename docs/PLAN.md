 ...
 *
 * @generated by sofabgen 0.1.0 on 2026-06-28T00:00:00Z
...
The generator owns a small **wire-format cost model** and computes the bound **analytically from the IR** — it does *not* run each target corelib (that would break the single, cross-platform `sofabgen` binary and need every language's toolchain at generation time). This is safe because the **wire format is identical across all corelibs**, so one language-independent calculation serves every backend. The model sums an **upper bound** per field (it bounds the encoder; it does not re-implement it):
...
- **Receive (decode) — incremental `feed()`:** the corelibs' `IStream` is a resumable decoder; generated decode exposes it as `decoder()` → `feed(chunk)` / `finish()`, callable repeatedly with **arbitrary chunk sizes (down to 1 byte)** — decoder state persists across calls until the message completes. **Verified by feeding one byte at a time**, against the real corelib, for C, C++, Rust, Java, C#, Zig and Dart. Three targets do not have it, for two different reasons:
  - **Go, Python** — their corelibs stream **pull**-shaped (the caller supplies a reader, the corelib pulls: `Decoder.Next` / `Decoder(reader)`). That is real, memory-bounded streaming, but it is not a push `feed`, and one cannot be synthesised over it without inverting control. They expose the pull shape instead (`deserialize(Decoder(reader))` in Python); a `feed()` needs a resumable decoder in `corelib-go` / `corelib-py` first.
  - **TypeScript** — `corelib-ts` *has* a resumable `IStream`, but the generated decode is a monomorphic pull cursor over a contiguous buffer and no visitor is emitted to drive the streaming path (ARCHITECTURE §9.3 family 6). Reaching it means generating a second, full decoder per type — tracked separately.
  A payload split across chunks is reassembled by whoever owns the split: the corelib (Dart, Java, C#) or the generated visitor (Zig, Rust). Where decode borrows from the input rather than copying — Zig's strings and blobs — the borrow holds only while a payload arrives whole in one chunk; a split one is copied, because there is no contiguous slice left to point at. *(Superseded: CORELIB_PLAN §6.7.1 withdrew the one-shot path's exemption, so no target borrows on either path any more — generator#412, ARCHITECTURE §10/§12.)*
- **Send (encode) — flush callback / sink:** when the output buffer is smaller than the message, the corelib invokes a **flush callback** as the buffer fills, emitting bytes in chunks and reusing the buffer (`buffer_set`). Generated encode surfaces this through `encode_to(ostream)` (spelling per language), which serialises **and flushes the tail**: C `_encode_to`, C++/Rust `serialize` into an `OStream` built with a flush sink, Java/C#/Dart `encodeTo` over a `FlushSink`, Python `serialize` into `Encoder.over_buffer(…, flush=)`, TS `serialize` into `new OStream(buf, off, flush)`, Go `EncodeTo(io.Writer)` — the encoder drains into the writer as its buffer fills, so the message is never held whole. `encode()` is the same path with a buffer big enough for the whole message, i.e. the convenience rather than the capability.
...
The config file format **MUST be described by its own JSON Schema**, shipped in the repo as `schema/sofabgen-config-schema.json` (alongside the message schema, `sofabuffers-schema-v1.json`). It is a first-class, maintained artifact:
...
codegen/
├── cmd/
│   └── codegen/              # CLI entrypoint (the sofabgen binary, §8.8)
│
...
The static parts (2–3, and the harness skeleton) come from per-language **project templates** in the generator; the message-aware parts (1, and the JSON⇄type mapping inside 4) are rendered from the IR. Emitting a root project is **config-controlled** (a `project`/`harness` emit option — see §7), so the same minimal CLI (`sofabgen --config … --lang …`) produces either just the sources or a full buildable project. The existing hand-written `test/codegen-examples/`, `test/example-cpp/`, and `test/capability-guard-*` are the **reference shape** these generated projects must match (and the seeds for the templates).

### 9.2 Test driver (the matrix runner)

For every `(definition, config)` pair the `runner/`:

1. runs the generator with an `emit: project` config to **generate the whole root project** (`sofabgen --config <cfg> --lang <lang> --in corpus/defs --out <lang>/`) — message sources + build files + devcontainer wiring + harness;
...
| **M2** | **First backend emits compiling code** | `sofabgen --lang c` turns `example.yaml` into C (`object.h`) sources that compile against `corelib-c-cpp`, **with capability guards** (§5.4). | 1 |
...
| **M8** | **1.0 release** | Reproducible/deterministic output is golden-tested; docs + per-language quick-starts published; the release pipeline ships cross-platform static `sofabgen` binaries (win/linux/mac × x86-64/arm64). | 3 |
