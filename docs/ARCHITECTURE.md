...
2. **Push child-visitor** (Go, Dart). The generated object drives the corelib's
   ...
   **event-equivalent** by construction rather than by agreement — same
   ...
   the receiver-side `max_dyn_*` limits are enforced in the same header hooks, one
   category over (`limitExceeded()`), for a scalar and a native array, and by the
   corelib collector for a wrapper array's elements (§9.5).
...
- The corelib reports what arrives — the count at the header, the index of the
  ...
  - **Dynamic (no `count`)** — the array legitimately grows to *highest present
    ...
    **Three backends could not take this generator-side**, because their gap-fill
    is not generated: **Go** (`sofab.StringSeq{Cap:}` and siblings), **Dart**
    ...
    cap through it would produce the forbidden category. Each needed a policy
    bound of its own beside it. **TypeScript** took it generator-side like the
    six, but on top of its flat-visitor rebuild (#395) rather than the cursor path
    being retired. Go's and Dart's collectors take one now (see below);
    **corelib-cpp#124 is still open**, so a pure-C++ wrapper array's element index
    and element length are the one shape in the family still bounded by the schema
    alone.

**The two families collapse into one, and the axis that mattered was not the one
they were named for** (generator#388, #402). There used to be a *generated visitor
guards* family and a *passed into the corelib decoder* family, and the second was
being retired on the reading that a corelib must not **check** a receiver cap.
CORELIB_PLAN §6.2.1 has since been amended (documentation#86) and settles the
opposite of the prohibition, not of the diagnosis:

> **Passing a limit in is not the codec holding one (normative).** The
> **provenance** of the number is fixed above; the **site of the comparison** is
> not. A corelib **MAY** take a limit as an argument and perform the check itself,
> and a port that does is conformant.

What a codec must never do is **hold** a limit, **default** one it was not given,
read an omitted argument as *unlimited*, or **clamp** to one — and, in particular,
answer a missing cap with the format ceiling (`ARRAY_MAX`/`FIXLEN_MAX`) and report
`LimitExceeded` against it, which would promise a limit to raise that was never
configured. A missing cap is a **caller defect**: `InvalidArgument` (§6.3).

So the retired family was never "corelib-side"; it was **global per decode**. A
`sofab.WithMax*`, a `DecoderLimits` or a `Decoder(max_dyn_*)` binds every field of
the message alike, including the schema-bounded ones §6.2.1 forbids it to touch,
and the raise that keeps those decodable loosens the cap for the *unbounded* fields
it exists to protect. Those three are gone. What replaces them is **per field**,
and that is the property that carries the fix:

- the cap is applied at **that field's own** count/length header, before the
  allocation it exists to prevent;
- **behind** the MESSAGE_SPEC §7.3 tag test, so a field whose wire type
  contradicts the declared one is skipped and never capped;
- as the **else** of the schema bound, never beside it — one number governs, and
  the category says which (`INVALID` for the schema's, `LimitExceeded` for the
  receiver's);
- **as configured**, with no raise: `max_dyn_array_count: 4` now coexists with a
  sibling's `count: 100000`.

**Where the comparison runs is decided by one rule, and it is a measurement.** Hang
the cap on a call generated code **already makes at that header**, so the compare
folds in beside a bound test already there. Introducing a call to hold the check
costs one to two and a half percentage points of decode and is not worth doing;
where no existing call carries it, the check stays in generated code. Same
generated source, callgrind Ir/op, `vehicle_telemetry` stripped of every bound:

| shape | cost of the cap |
|---|---|
| Go, decoder-level options (`sofab.WithMax*`) | +3.14% |
| Go, per-field collector argument | ≈0%, not resolvable |
| C++, a guard in front of `readString` | +3.39% |
| C++, the cap passed **into** `readString` | +0.11% |
| Zig, generated guards | +5.10% |
| Zig, the cap on `arrays.allocN`/`grow`/`setElem` | +1.00% |

That rule produces a **split by field kind**, and the split is deliberate:

| field shape | where the comparison runs | why |
|---|---|---|
| wrapper-sequence array (index, element length, matrix row count) | **the corelib collector** — Go, Dart, TypeScript | generated code already hands the whole array over; the collector is the only thing that sees an element's id and length word |
| native array count, scalar `string`/`blob` length | **the generated header hook** — every backend | the hook is entered per field anyway; the only corelib call left on a scalar's path is the payload accumulator, and it is one callback too late (below) |
| every count/length word, in Python | **the `Decoder`**, handed all three caps per decode | corelib-py compares at the header and spends the verdict on any field `on_schema_bound` declares or `on_field` skips, so a per-decode number is exclusive with the schema after all |

**Why a scalar's cap does not ride on the accumulator.** It is the obvious
candidate — `PayloadAcc.Take` (Go), `acc.feed` (Rust), the Java/C# twins — and it
runs for every string and blob. But it is reached from the **payload** callback,
not the length header: the codec calls it when payload bytes arrive, and for a
message that truncates immediately after an over-cap length word it is never called
at all. §6.2.1 requires the check at the count/length header "for the same reason
`INVALID` is decided there (§5.2.3)", so moving it there would turn a
`LimitExceeded` into an `INCOMPLETE` on exactly the input the cap exists for — and
would buffer before rejecting. Verified on generated Go against corelib-go: the
three bytes `02 a2 06` (an unbounded `string` at id 0 announcing 100 bytes under a
cap of 24, then end of input) answer `decode limit exceeded` today, and would
answer `INCOMPLETE` with the check on `Take`. §9.5.2 records the same finding for
Rust, where it is also why the collector shape is unavailable.

**What a wrapper array hands over.** Its elements never reach the generated
visitor: neither the element index nor the element's length word. Go, Dart and
TypeScript hand the whole array to a corelib collector, so both of that shape's
receiver bounds ride **on the collector**, beside the schema bounds they are
exclusive with — the element index, the element byte length, and a matrix row's own
element count (corelib-go#134, corelib-dart#90, corelib-ts#164/#165). Python's
gap-fill is generated, and its element INDEX is the one number its `Decoder`
cannot see — an index is a field id, not a count word — so that single cap stays in
the array scope's own `on_field` arm. Either way the number comes from generated
code.

**The caps are constructor arguments, not settable fields.** corelib-go#134 turned
`&sofab.StringSeq{Out: …, Cap: …}` into `sofab.NewStringSeq(out, bounds, caps)`,
with the fields unexported, and corelib-dart#90 makes all three `required`. A
...
a compile error at the generated call site rather than a runtime one. Generated Go
carries one package-level `_caps` (`sofab.Caps{ArrayCount:, StringLen:, BlobLen:}`)
and one `sofab.Bounds` per axis at each construction: three numbers repeated at a
dozen sites read as three independent policies, and they are one deployment's.
Every entry is filled even where the schema bound beside it makes it inert, because
there is nothing for an omitted one to mean.

**Three things the move closed, and one it opened.**

- **A skipped field is never capped** (§6.2.1: it allocates nothing). A
  decoder-level cap could not honour that — it saw a count header, not a schema —
  so an over-cap array at an id the receiver never reads failed the decode. It
  now stays COMPLETE. That was generator#410, and it was the same defect in
  Python one layer up: the caps sat in the *else* of `on_field`'s tag chain, per
  SCOPE, where they also fired on an unknown id and on a header whose wire kind
...
- **A matrix ROW's own element count** was bounded by nothing the generator
  ...
  the floor where the collector was built. It now travels as the row's own
  `sofab.Bounds` (INVALID, §7.1) with the same `max_dyn_array_count` behind it —
  a **new rejection** inside a change that removes checks. One cap serves both
  axes: §6.2.1 states one `max_dyn_array_count`, and both axes are arrays.
- **A wrapper string/blob element's byte LENGTH** had no collector bound at all
  and lived off the decoder-wide cap.
- **Opened, in Dart only: the destination of a skipped field.** corelib-dart's
  `onBytesDest`/`onArrayDest` defaults allocate a destination sized from the
  wire, which is right for a hand-written visitor and wrong for a schema-bound
  scope: an id it does not bind, or one whose wire kind contradicts what it
  binds, was materialized from the wire count with only the decoder's cap in
  front of it. Every generated Dart visitor now overrides both and returns `null`
  for every id it does not bind — scopes with no array or payload field included.
  That is a tighter bound than any cap (nothing is allocated, not "at most N"),
  and it is the guard for the one shape §6.2.1 calls uncappable *because* it
  allocates nothing. Go needs none (an id with no arm materializes nothing);
  Python's is the corelib's `headroom` sizing, which bounds an undeclared array's
  destination by the bytes actually fed rather than by the count claimed.

**The per-backend history**, kept because each half went in separately:

- **Python, generator#325/#406, then back into the codec** — the guards went into
  `on_field` first, cap emitted as configured; #406 moved the schema bound itself
  into corelib-py's `on_schema_bound`, which is what closed the §6.2.1 half (a
  native array's count used to be checked in `on_array_begin`, leaving its id in
  the *else* where the array cap sat, so a `count: 8` array under a configured
  `max_dyn_array_count: 4` was rejected at a wire count of 6). See §9.5.1 for what
  that move cost. **corelib-py#137 then made the three caps required arguments of
  `Decoder(...)`**, and with `on_schema_bound` already there the codec can be
  exclusive with the schema, so the comparison moves back down: generated Python
  states the numbers and keeps only the wrapper element index. What it owes in
  exchange is §9.5.1's second half — the two declines that keep the codec's cap
  off a field it must not reach.
- **TypeScript, generator#388/#405/#407** — the first per-field move: the cap
  became the else of the schema bound in `fixlenBegin`/`arrayBegin`, the wrapper
  array's two bounds went onto corelib-ts's collectors, and the module-wide
  `DecodeLimits` went with the raise it needed.
- **Go and Dart** — blocked until their collectors could carry a policy bound,
  because `cap`/`ElemMax` there mean the *schema* bound and answer INVALID:
  routing a receiver cap through them would have produced the category §6.2.1
  forbids, and retiring the options first would have left wrapper elements
  unguarded, which is strictly worse than the imprecision being fixed.
  corelib-go#134 and corelib-dart#90 land the collector arguments, strip the
  codecs of the numbers they had invented, and remove the format-ceiling fallback
  each had been reporting `LimitExceeded` against; these are the generator halves.
  Go's unused `SchemaBoundVisitor` — the hook that kept a decoder cap off a
  schema-bounded field — went with the cap it existed to exempt.
- **Rust std is exempt** for a structural reason: §9.5.2. `rs-no-std`, `c` and
  `cpp (corelib: c-cpp)` are out because no schema-unbounded field can exist
  there at all.
- **Pure C++ is the one shape still open**: corelib-cpp#124 / #420 — a pure-C++
  wrapper array's element index and element length remain bounded by the schema
  alone.
