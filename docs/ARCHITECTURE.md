...
2. **Round-trip harness** — `emit: project` builds the generated code against the
   ...
   **TypeScript, Python, Dart**. Widening an fp32 sNaN into a double is where
   ...
   different loss. ts and dart each carry a shell block over their harness
   `recode` verb after a real defect (#235, #226); python has neither the verb
   nor the defect, so its leg is `tests/conformance/python/fp32_nan_check.py`
   (generator#414), an in-process driver over the already-generated example
   project which also drives the **streaming** surface — §6.5 asks for the
   outcome on *every* decode surface, and neither of the other two blocks covers
   it. It runs inside the engine loop and re-asserts `sofab.IMPL` in its own
   process, because corelib-py's pure and native halves narrow and widen fp32
   with separate code.
   ...
   **and** any materialized walk" — and calls the second one out by name: "a
   ...
   class this section exists to prevent". The python driver runs both: it
   re-encodes, and it requires the materialized `msg.somefp32` /
   `somefloatarray[i]` to be the fp64 image of the wire word, sign, quiet bit
   and 23-bit payload intact. The two coincide today — the dataclass value
   *is* what the re-encode narrows — and stop coinciding the moment a target
   stashes raw bits beside a canonical value, which is exactly what §6.5 tells a
   double-only port to build.
   ...
   which both the ts block and this driver assert. And the python leg pins an
   **outcome, not a mechanism**: generated python overrides no raw-bits hook
   ...
   normalizes (measured, both ways: a generated visitor that quiets every NaN
   turns 6 of the 9 fixtures red on both engines and on both oracles — 60
   failures — and a corelib copy whose `_pack_f32_bits` returns `0x7FC00000`
   turns the same six red on the pure engine, on the re-encode oracle only, the
   value oracle staying green, which is the two legs being independent). The
   eight native-`fp32` targets are untested for the same rule; §6.5 says the
   natural implementation meets it on its own, which is an implementation note
   rather than a coverage exemption. The concern is *not* blocked on a
   family-wide change: it applies to the three double-only targets, two of which
   already emit the wire → wire `recode` verb, so #468 tracks emitting it from
   `generators/python/project.go` and folding the three one-shot legs onto one
   `tests/conformance/lib/` driver — leaving suite-local only what a `recode`
   verb cannot reach, python's two engines and its streaming surface.
