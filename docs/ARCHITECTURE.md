...
| **M8 reproducibility + release** | **done** — golden-output test (regenerate scalars.yaml for every backend, byte-diff vs committed goldens); `.github/workflows/release.yml` builds static `sbufgen` binaries for win/linux/mac × amd64/arm64 on a `v*` tag; README quickstart added. |
