...
highest `pos`. Those two widths are what §1 **bounds the field by**: a wire value
outside the implied width is INVALID, and one inside it is valid even where the
schema names no constant for it or it sets an undeclared bit. A target MAY hold
the field wider than that — several do — and must then enforce the width as an
explicit check, because its storage will not (see the "Decode verdict"
subsection). `sequence` is a wire type only — there is no `sequence` keyword in
...
- **Static helpers belong in the corelib; generated code stays thin.** A corelib
  ...
  `CheckIndex` / `ReserveRow` in corelib-cs, and `sofab::seq::place_elem` /
  `reserve_elem` / `reserve_row` / `check_index` in corelib-rs and
  corelib-rs-no-std) are the shape to copy — one collector in the corelib, the bounds passed per field; and
...
- **Heap-free field storage is its own axis — offer it wherever the language can
  ...
  1. **It is orthogonal to the corelib/environment.** It selects storage, never
     ...
     crate can hold its bounded fields in `heapless` while keeping serde and the
     ordinary std prelude (§10, Rust row).
...
1. **Flat visitor + location-stack** (Rust, C#, Java, Kotlin, Python, TypeScript, and
   ...
   **Optional bulk element hand-off (Java and Kotlin, `Visitor.arrayBulk`/`arrayBulkEnd`).**
   ...
   keeps the capped-reservation, grow-as-you-go fill (#96). On both `java` and
   `kotlin` the offer covers **every** integer element, `enum` and `bitfield`
   included: those two are backed by the primitive whose width their declaration
   implies (MESSAGE_SPEC §1), so the destination states the declared width exactly
   as it does for a `u8`, and the corelib enforces it (generator#516; see
   "Decode verdict"). On `kotlin` the destination handed over is always the SIGNED
   array — an unsigned one is an inline class over it, so `asByteArray()` is a
   reinterpretation of the field's own storage — and the corelib takes the
   signedness from the array header's `ArrayKind`, not from the destination type.
   ...
   **That independence is now the spec's rule, not just this design's
   ...
   the exact declared width (`byte[]`/`short[]`/`uint[]`/`ulong[]`) — for an
   `enum` and a `bitfield` too, at the width their DECLARATION implies (§1), so
   an `array<enum>` over `{0,1,2,10}` is an `sbyte[]` exactly as an `array<i8>`
   is, and only the `boolean` array stays a `List<bool>` (its wire element is a
   `u8` and its member is not, so there is no shared width to put it in) — Rust
...
4. **Child-visitor** (pure C++ `corelib-cpp`). Nested objects decode via
   ...
   **The decoder is also resumable, which decides what a decode destination may
   ...
   - a **boolean array**'s member element type is `std::uint8_t` on every `cpp`
     profile. `std::vector<bool>` could never have been a destination: it is the
     bit-packed specialisation and has no `data()`. The byte storage must not make
     the decode a `u8` one, though. CORELIB_PLAN §4.4 reads every non-zero element
     as `true`, normalizes it to `1`, and bounds no element by its width, while a
     `u8` read keeps `2`, and refuses (c-cpp) or silently masks to `false`
     (corelib-cpp) `256` (generator#581). So on both legs the member binds through
     `sofabgen::RawArray<Container, bool>`, the enum view below with `bool` as the
     element type. Each corelib's container-of-`bool` read (corelib-c-cpp#172,
...
5. **Descriptor-table callback** (C `corelib-c-cpp`). A static descriptor table
   ...
   **sized blob**: an opaque byte field can be shorter than its `maxlen`, and a
   ...
   A `boolean` is described as `SOFAB_OBJECT_FIELDTYPE_BOOLEAN`, and a boolean
   array or row as `_ARRAY_BOOLEAN` (corelib-c-cpp#172), never as `UNSIGNED`.
   The wire form is the same unsigned varint, but only the boolean tags make the
   corelib apply CORELIB_PLAN §4.4: every non-zero value is stored as `1`, and no
   width bound applies. Described as `UNSIGNED`, the one-byte `uint8_t` member
   kept `2` raw and refused `256` as `INVALID` (generator#581).
...
- **A native array is filled through a destination, and that is the only way its
  ...
  **The member IS the destination, because the member is a typed array**
  ...
  array is offered the member itself as `f32`. That destination is bit-exact since
  corelib-ts#188: the corelib stores every non-NaN as a value (an fp32 survives the
  double exactly) and a NaN by its wire word, so a signaling NaN is not quieted
  (§4.6/§6.5). It replaced a `bits` view over the member's buffer, which cost a
  `Uint32Array` per array per decode, and reading `.buffer` moves a small on-heap
  typed array's storage off the heap. Older corelibs quiet an sNaN through `f32`,
  so this output needs corelib-ts#188 or later. The member holds the wire words, so
  there is no fp32 **array** raw-bits companion and `writeFp32Array` copies them
  straight out. The scalar half stays — a JS number has nowhere to keep the bits.
  **Every empty typed array is one shared instance per type.** A module declares
  `const _E_<Type> = new <Type>(0)` for each typed-array type it uses and puts it
  wherever an empty one is needed: a native array member with no schema default,
  a matrix row's gap padding, the visitor's per-array registers and each reusable
  bulk target's initial slot. A fresh instance per slot was about 20 allocations
  per decode of a message with ten native arrays. Sharing is sound because a
  zero-length typed array cannot be written through or resized, decode REPLACES a
  member with `new <Type>(count)` instead of filling the one it holds, and nothing
  generated compares arrays by identity (the omit test and `isDefault` go by
  length or `elementsEqual`). The one visible consequence is that two fresh
  messages' empty array members are the same object. A declared default stays a
  fresh array per message.
...
- **Every wrapper array is collected by the corelib** (generator#587). `StringSeq`
  and `BlobSeq` own a `string`/`blob` element whole — the index rules, both bounds,
  the payload join and the strict UTF-8 decode. A *framed* element — a struct, a
  union, a nested row, a matrix row — takes the two index bounds and the gap fill
  from the corelib as well, through `reserve(id)` / `place(id, value)`, and what
  stays generated is the routing the shape needs: the index register, the scope
  switch, and the destination it names. The `_ObjSeq` / `_MatSeq` / `_RowSeq`
  classes the child-visitor shape needed are gone with it: a flat visitor routes
  the element scope itself.

  **Which collector an element takes is decided by one thing: whether its gap value
  may be SHARED.** A decoded wrapper array grows to the arriving index (§9.5 shape
  B), so a gap an omitted interior element left has to be filled with the element
  default. `ElementSeq<T>` writes one shared `def` into every gap, which is right
  for `""`, for a zero-length `Uint8Array` and for the module's shared empty typed
  array a native matrix row pads with — a zero-length typed array holds nothing,
  drops an indexed store and cannot be grown. It is *wrong* for a struct or union
  element, a wrapper row, and a `Long[]` matrix row under `int64: long`/`number`:
  those defaults are fresh mutable objects the caller can reach, so one instance
  would alias every gap of the array onto it and — since an arriving element decodes
  into the slot the reservation placed — every *element* onto it too. Those take
  `FramedSeq<T>`, `ElementSeq`'s twin with a `make: () => T` **factory** in `def`'s
  slot. The factory itself stays generated: it names a generated type, which is §8's
  own line between the layers, and it is emitted once per element type as a
  module-level `const` rather than a closure per array occurrence.

  Two more static helpers left the backend in the same move: the fp32 raw-bits
  companion (`fp32RawBytes`, the §4.6 channel) and the `Long` flavour of the
  default compare (`longElementsEqual`) are corelib-ts functions now, so a
  generated module declares no top-level `function` at all. One function and not
  the `into(out, off, bits)` pair the backend carried: that second flavour existed
  for the fp32 *array* raw path, which §6.7 retired before this move, so moving it
  would have put a function in the corelib's public API that no generated module
  in the family calls — and API nothing calls is API that drifts.

  **What it cost, measured END TO END** (Callgrind, subtract method, §15,
  `vehicle_telemetry`). §8 makes a cost in instructions/op on a maxspeed target one
  of exactly three overrides that can block a helper move, *measured and never
  asserted* — so this is the number the move turns on, and the A leg has to be the
  state before **both** halves: generator `cb28dc5` with corelib-ts at the
  `origin/main` it had then.

  That is worth spelling out because the first reading here got it wrong. An A
  that holds the corelib at the NEW commit and reverts only `generators/typescript`
  measures the generator half of a two-half step, and on this target it reads about
  0.4 % high on decode — the corelib half is not inert under the old generator,
  because `StringSeq` / `BlobSeq` take their own element index through the same
  shared rule, so a change to that rule reaches them with no codegen change at all.
  A two-half step is measured against the state before both halves or not at all.

  | row | encode, A → B | decode, A → B |
  |---|---|---|
  | `ts-bigint` | 586 113 → 585 863 (−0.04 %) | 787 331 → 792 697 (**+0.68 %**) |
  | `ts-long` | 579 934 → 579 935 (+0.00 %) | 781 459 → 786 877 (**+0.69 %**) |
  | `ts-number` | 595 682 → 595 905 (+0.04 %) | 776 923 → 782 225 (**+0.68 %**) |

  Encode is flat, within the ±0.04 % this harness spreads over for these rows
  between builds — which is what a move with no encode path of its own has to
  read. The decode cost buys the §6.2.1 verdict rule and the §9.5 growth ONE
  implementation for the family instead of one per array occurrence per schema, and
  it is flat across all three `int64` representations.

  **Why it is that small, and what the first shape cost.** The `ts` rows measure
  the V8 **baseline** tier (`--max-opt=1`, and `tests/bench/lang/typescript.sh` says
  why), where nothing is inlined and a CALL is the expensive thing — about 200 Ir
  of it. On this target a helper move is therefore priced per call and not per line
  moved, and the first shape of the collectors made three calls per element
  (`reserve` → `checkIndex` → a shared `overIndex`) plus one per construction. It
  measured **+2.3 % on every row**, over the 2 % the move was given as its budget.

  What removed it was not taking the rule back out of the corelib. It was reading
  §6.2.1 once more: "never both" means exactly ONE of the schema `count` and the
  receiver cap is ever compared against, so which one is in force is known at
  construction. `indexBound` picks it there, every entry point is then the single
  `id >= this.bound` that remains, and the shared `rejectIndex` — which still owns
  the whole which-verdict rule, in one place — is reached only on the path that
  throws. The rule stayed exactly as shared as it was; what went away was paying
  for the sharing per element. The ~26 calls a `vehicle_telemetry` decode still
  adds account for the residual 5 300–5 400 Ir almost exactly.

  If a later change needs more of it back, the two fallbacks in order are hoisting
  the collector to a visitor field with a `bind(out)` instead of constructing one
  per array occurrence, and dropping the matrix row's duplicated `checkIndex` by
  taking its count bound first.

  `tests/bench/results.txt` is NOT updated by this. The numbers above come from a
  `--rows ts-bigint,ts-long,ts-number` run against a corelib-ts commit that is not
  on its `origin/main` yet, and a partial run's header cannot attribute the rows it
  did not carry (§15), so committing it would trade a stale cell for a false
  provenance. The `ts` decode cells move by more than the 0.3 % hysteresis band, so
  they are stale until the corelib half lands and a FULL run is taken — the B
  column above is what that run should read.
...
- **Whole-unit / measure-then-deliver corelibs** surface the truncation
  ...
  - **Dart** — corelib-dart makes ONE call per aggregate, at its header
    (`onString(id, length)`, `onBlob`, `onUnsignedArray(id, count)`,
    `onSignedArray`, `onFp32Array`, `onFp64Array`; corelib-dart#96, generator#570),
    and the generated arm calls `invalidate()` before it returns the field's
    destination, so the over-bound verdict is taken at the count/length word. The
    call is chosen by the WIRE kind and an arm exists only in the call of the
    declared one, so the subtype gate is structural rather than emitted.
...
- **dart** carried it as `MessageVisitor.onArrayElemBound(id, kind) -> ElemRange?`
  until corelib-dart#96 folded it into the destination: the field's
  `InlineInt64Array` is constructed with `range: const sofab.ElemRange(lo, hi)`,
  and the codec applies it to every element as it is decoded, on both surfaces
  (generator#570).
...
- **`enum` and `bitfield` are bounded by the width their DECLARATION implies, and
  they sit outside `ir.NarrowRange` for that reason.** MESSAGE_SPEC §1 (doc
  `382159e`) derives their bound from what the schema declares rather than from
  the Kind: an `enum` is bounded by the smallest **signed** type holding every
  declared constant (`i8`..`i64`), a `bitfield` by the smallest **unsigned** type
  holding its highest declared `pos` (`u8`..`u64`). Constants `{0, 1, 2, 10}`
  imply an `i8`, so the field carries `-128..127`; positions `0, 1, 3` imply a
  `u8`, so the field carries `0..255`. A value INSIDE that width is **valid**
  even when the schema names no constant for it and even when it sets a bit no
  flag declares; only a value outside it is malformed input and **MUST** be
  reported INVALID (§7.1), exactly as an over-width `i8` is. The enum's width is
  signed whatever its constants are, because the *wire type* is (§1, CORELIB_PLAN
  §4.5): deriving signedness from the constants would make the encoding a
  function of them, so adding a negative constant later would change the bytes
  rather than only the range.
  - **Undeclared bits are NOT masked away.** Masking would turn malformed-looking
    input into a DECLARED combination and report it `Ok`, which is precisely the
    outcome §1 forbids. Whether an undeclared value survives a
    decode-and-re-encode round trip is a storage question §1 leaves to the target.
  - **A set/mask reading was withdrawn, and the shapes below are what replaced
    it.** MESSAGE_SPEC carried it for six days (documentation PR #89): an enum
    bound by its SET of constants, a bitfield by the MASK of its positions. Under
    it `5` against `{0, 1, 2, 10}` and `4` against positions `{0, 1, 3}` were
    INVALID; both decode now. §1 gives the channel as its reason, and that is why
    it shows in the emitted code: an array's elements are consumed inside the
    corelib loop, so a bound must cross that boundary as an **interval**, and a
    width fits where a set does not. Two consequences follow for schema authors —
    adding a constant or a flag is not a breaking change for a decoder, and a
    receiver still never stores something its declared type cannot represent.
  - **The bound changed; the need for a check did not.** Dropping the guard along
    with the set reinstates generator#513: `1000` into an `i8`-backed enum decoded
    as `-24` and `256` into a byte-backed bitfield decoded as `0`, both with the
    verdict `Ok`.
  - **Two facts, one source.** `ir.EnumWidthRange` and `ir.BitfieldWidthMax` live
    in `internal/ir/width.go` beside `ir.NarrowRange`, so the three width bounds
    are derived in one place and cannot drift between targets. Both answer
    `ok == false` where the implied width IS the 64-bit accumulator the value
    arrives in — an enum needing `i64`, a bitfield whose highest `pos` is 32 or
    above — and a backend then emits **nothing**, because no reachable value can
    breach it. An enum never reaches that elision from a valid schema: the
    validator caps a constant at signed 32 bits, so the widest declaration implies
    `i32`. An enum declaring no constant, and a bitfield declaring no flag, take
    the narrowest width rather than a special case: every signed width contains
    `0`, so folding it into the span can never widen the answer.
  - **Storage is never the bound, even where the two coincide.** §1 lets a target
    hold the field in the smallest integer covering the declaration, and `c`,
    `cpp`, `csharp`, `go`, `rust` and `zig` do — guard and member are then the
    same number, still reached by two routes, because a bound read off the member
    would silently follow a storage decision. The other five hold it wider (`java`
    in a `long`, `kotlin` in an `Int`/`ULong`, `dart` in its 64-bit `int`,
    `python` in an unbounded `int`, `typescript` in a `number`/`bigint`), and §1's
    fourth consequence binds exactly them: a receiver that cannot hold the field
    at the declared width **MUST** enforce that width as an explicit check,
    because nothing about its storage will.
  - **The check runs on the RAW accumulator, ahead of the narrowing cast.** That
    cast is itself the mask §7.1 forbids — `value as i8`, `@intCast`,
    `static_cast<T>`, `.toInt()` — so a comparison placed behind it can never see
    anything.
  - **All six positions, both kinds.** A value can land as a scalar field, a
    ...
    union member, or a matrix row element: twelve stores. Several of those share
    one emitted arm per kind, which is exactly why each is pinned by name in the
    backend tests — a fix covering only the scalar is a third of the job.
  - **Where the elements are consumed inside the corelib, the bound travels there
    as the interval it now is.** `cpp` hands `sofab::readArray` an explicit
    `sofab::ElemBound{-128, 127}` / `{0, 255}`; `go` passes the pair to
    `sofab.NewSignedMatrixSeq` / `NewUnsignedMatrixSeq`, exactly as a `u8` or
    `i16` row already did; `dart` constructs the field's `InlineInt64Array` with
    `range: const sofab.ElemRange(-128, 127)` and arms `sofab.IntMatrixSeq`'s own
    `lo`/`hi`; `python`
    returns `(None, -128, 127)` / `(None, None, 255)` from `on_array_begin`.
    Because each hook carries the WHOLE bound, an out-of-range element is refused
    **at** the element, so an array a truncation cuts short behind it is INVALID
    and not INCOMPLETE (§5.2). A scan of the assembled list cannot reach that
    case — it never runs for an array that does not arrive — so no such scan is
    emitted.
  - **Two generated collector shapes went with the set.** A matrix row's values
    never reach a generated store: the collector gathers the row and places it. To
    state a SET there, `go` emitted a `_row<T>` wrapper embedding
    `sofab.*MatrixSeq` and `dart` a subclass of `sofab.IntMatrixSeq`, each
    overriding the one element callback and delegating the placing. A width is
    what the collector's own `lo`/`hi` pair already carries, so both are gone and
    the row takes the ordinary native path.
  - **One position in the family is still not reached, and it is a corelib
    limit.** `cpp` with `corelib: cpp`, **bitfield** matrix row element:
    `sofab::MessageSeq` places the row and reads it with an unbounded
    `sofab::readArray`, passing no element bound and offering none to pass, so
    `256` into a one-byte row is `0` — an in-width value — before any generated
    line could look. Closing it needs an element bound on `sofab::MessageSeq`. A
  ...
  - **`corelib: c-cpp` needs nothing from its corelib, and that is the clearest
    illustration of the rule.** That backend's `read` binds the destination by
    ADDRESS and the C runtime fills it after the field callback returns, so
    generated code never holds the value; the only bound
    `sofab_istream_read_field` carries is `sizeof(destination)`, its option word
    being wire type, fixlen subtype and string termination and nothing else. No
    set or mask could cross that interface — but a width does not have to: this
    backend stores both kinds at exactly the implied width, so the destination
    size **is** the §1 bound. Measured: both `c-cpp` labels enforce all twelve
    stores, the two matrix-row cells included. `c` is conformant for the same
  ...
  - **A bulk element offer is a question about the DESTINATION's width** (`java`,
    ...
    **`java` and `kotlin` both give the two kinds the narrow destination and take
    the offer.** An `array<enum>` over `{0,1,2,3,4}` and an `array<bitfield>`
    whose highest `pos` is 4 are backed by the width their declaration implies,
    derived by `ir.EnumWidthRange` / `ir.BitfieldWidthMax`: a `byte[]` for both on
    `java`, a `ByteArray` and a `UByteArray` on `kotlin`. The destination
    therefore states `i8` and `u8`, and `IStream`'s `narrowI8`/`narrowU8` refuse
    an element past it with `INVALID_MSG` — on the element loop and on the
    byte-at-a-time path both, so a chunk boundary does not change the verdict.
    Where the implied width is the accumulator's own (a bitfield whose highest
    `pos` is 32 or above) the destination is 64 bits wide, which is again the
    declared width and carries no bound to lose — the position `array<u64>` has
    always been in. The generated per-element guard stays emitted behind the
    offer, because `Visitor.arrayBulk` defaults to `null`: a corelib that declines
    it fills the same array through the callback, and the bound holds either way.
    The two ARRAY cells of this rule therefore exercise the corelib's half on both
    targets, and the other ten exercise generated code.

    The **signedness** does not travel with the destination on `kotlin`: an
    unsigned array is an inline class over its signed peer, so what `arrayBulk`
    hands back for a `UByteArray` field is that field's own storage viewed as a
    `ByteArray`, and the corelib picks `narrowU8` over `narrowI8` from the array
    header's `ArrayKind` — which `arrayWireKind` derives from the same
    declaration. Verified rather than assumed, against `corelib-kotlin-mp`
    ...
    **`csharp` narrows the same two destinations and the enforcement does NOT
    ...
    `sbyte[]`/`byte[]` at the implied width for the same reasons Java's are — one
    `.ToArray()` plus one `Array.ConvertAll` per field per encode, and an
    `Add`-grown list on decode where the wire count was already in hand — but
    `corelib-cs`'s `IVisitor` has **no bulk element offer at all**: every element
    arrives through `Signed(int, long)` / `Unsigned(int, ulong)` in the 64-bit
    accumulator, and the generated arm narrows into the array itself. So the
    generated per-element guard is the whole of the bound at those two cells and
    may never be traded for the cast behind it — the position every backend
    without such an offer is in.

    **What all this costs is measured, not asserted.** `tests/bench`'s
  ...
  - **Rollout.** All eleven backends enforce the bound. Ten emit it: `rust` (all
    ...
    through its descriptor without emitting anything, as above. Every one of the
    ten reaches all twelve stores for a message that ARRIVES whole; the single
    element none of them can see — `cpp`'s pure-corelib bitfield row — is the
    corelib gap listed above.
  - **`kotlin` is the one target that had a bound before this rule, and it was the
    wrong one.** It checked an enum against the signed 32-bit range — the *wire
    type's* ceiling, which happens to coincide with the `Int` a scalar enum is
    held in, so the bound and the storage looked like one fact. §1 settles exactly
    that confusion: the field is bounded by the width its constants imply, not by
    its carrier's range, and the bitfield beside it, which had no check at all, is
    bounded by the `u8` its highest `pos` implies. Its ARRAY positions now hold the
    element at the implied width, which is the same two facts coinciding again —
    and the guard stays emitted there all the same, because what makes the storage
    narrow is a choice this backend made and not what the rule rests on.
  - **The `go` defect that sat next to this rule is closed (generator#609).** A
    wrapper-array element was materialised by `sofab.NewMessageSeq`, which
    appended a ZERO value, and generated code applied no defaults to it -- nor to
    a nested struct field at all: `New<Msg>` seeded the message's own fields
    only. The struct's `Serialize` compared its members against the SCHEMA
    defaults, so a fresh message was not empty on the wire, an absent nested
    member decoded as 0, and an interior element omitted for equalling its
    default (§5.1) came back as 0 (`[9, 1]` → `[0, 1]`). Every struct/union type
    whose members declare, at any depth, a default other than Go's zero value
    now gets an unexported `setDefaults()`; `New<Msg>` calls it per such field,
    and a struct/union array hands it to corelib-go's `NewMessageSeqInit`
    (corelib-go#147), which seeds every slot it creates -- the gap and the slot
    at the id -- and never re-seeds a reopened one (§7.4). The gap fill stayed in
    the corelib (§8: the loop is schema-independent; only the defaults are the
    schema's), which is why this is not the generated collector wrapper this
    paragraph once proposed. A type whose declared defaults are all zero gets no
    method and keeps `NewMessageSeq`: its zero value IS its default, and the
    bench schemas generate byte-identical Go. The corelib's `init != nil` test
    costs +24 / +18 Ir per decode on the `go` / `go-unbounded` rows (45 056 →
    45 080, 39 526 → 39 544), inside the results.txt hysteresis.
...
- **One schema-validity rule sits beside the decode bound, and it is about naming
  rather than about validity.** An `enum` field **MUST** either declare a
  `default` naming one of its constants, or belong to an enum declaring a constant
  with the value `0`. `checkEnumField` in `internal/parser/validate.go` enforces
  it for every id scope, and `schema/README.md` §4 carries it as the
  absent-`default` half of `defaultMatchesEnum`. It follows from §2 and §4 rather
  than from §7.1: §2 initializes a field with no `default` to its type's zero
  value and a sparse encoder omits the field at exactly that value, so absence
  reconstructs `0` — a value the declared width admits, but one the schema gives
  the author no name for unless a constant carries it. It binds a **field**: an
  array of enum needs no counterpart, because `count` is a capacity and nothing is
  padded to it (§11), so an array with no `default` initializes empty rather than
  to a run of zeros. A `bitfield` needs none either — its zero is "no flags set".

  **An `enum` declaring NO constant is refused outright**, in `validateEnumDef`.
  A declaration that names no value is not a usable type: no field of it could be
  given a default, and every value it decoded would be unnameable. The bitfield
  twin is NOT an error — an empty `bits` map declares no flag, its implied width
  is `u8`, and `0` is the combination it does state.

  The same distinction governs an authored **array-of-bitfield element default**,
  which `internal/parser.checkMaskElem` compares against the DECLARED MASK
  (`v &^ mask`) and not against the implied width. That is deliberately narrower
  than the decode bound and deliberately not it: an element default setting a bit
  no flag declares is a combination the schema offers the author no way to name or
  read back, while the same value arriving on the wire is one every decoder in the
  family accepts.
- **`cpp` needed a different shape from the rest.** corelib-cpp's typed `read()`
  ...
  A scalar `enum` and `bitfield` take that same shape for their declared-width
  check — including the `if (is.read(_v))`, which those two used to skip, writing
  the zero-initialized temporary into the member on a §7.3 skip.
...
`ElemBound{lo, hi}` instead of the type-derived one — the width their declaration
implies, which the type of the destination container does not state (§1,
generator#516) — and the argument carries it whole, so nothing is left for a
second pass over the assembled array. Floating-point elements are excluded by construction: instantiating
...
`rust` std now has a schema-sized stack too: a fixed `[_Loc; D+1]` plus an `sp`,
where D is the depth of the deepest frame (one per sequence_begin that enters a
live scope). It replaced a `Vec<_Loc>` that allocated at the first
`sequence_begin` of every decode — the one heap allocation left in an
`allow_dynamic: false` decode. D+1 is exact, not slack: with `cur` on a depth-d
frame the stack holds d entries, and the sequence opened there — deeper frame or
skipped subtree — pushes the (d+1)th. Only an over-index / over-cap wrapper element
(`inv`/`lim` already set, both dominating `err` at every verdict) keeps `cur` on
its array frame and so can push further; the push is a `get_mut`, and its overflow
takes the no_std arm — `err`, and the level entered as a skipped subtree — never an
out-of-bounds index. `tests/conformance/rust/decode_stack_depth.rs` pins the size
from both sides (the longest well-formed chain decodes; a stack one short answers
`BufferFull` on it).
...
- **go** — `sofab.UTF8Valid(bytes) bool`, called in every arm that stores a
  ...
  build configuration.
- **dart** — the destination now comes first by construction: since
  corelib-dart#96 the visitor answers `onString(id, length)` with the field's
  `InlineString` (or `null` to skip), and the codec validates the bytes once the
  payload is whole — only for a destination it was handed, so a skipped string is
  never inspected. Generated Dart carries no UTF-8 code at all; the schema
  `maxlen` is a *byte* bound and is judged against the announced `length`.
...
put a string — dart gets it from the corelib: since corelib-dart#96 every
`MessageVisitor` default, aggregates and sequences alike, is "skip", so a scope
with no string destination simply overrides nothing (`sofab.VisitorBase` carried
it before that, and an emitted base before corelib-dart#65);
...
the same cap, and that growth is the corelib's in every one of them — no backend
emits it (generator#587): `for len(*s.out) <= int(id)` in corelib-go, `rcap`-bounded
collectors in corelib-dart, `reserveElem`/`placeElem` in corelib-zig, corelib-java
and corelib-kotlin-mp, `reserve_leaf`/`reserve_elem`/`reserve_row` in corelib-py,
`Seq.PlaceElem`/`ReserveElem`/`ReserveRow` in corelib-cs, `seq::place_elem`/
`reserve_elem`/`reserve_row` in corelib-rs and corelib-rs-no-std,
`ElementSeq`/`FramedSeq` in corelib-ts, `MessageSeq`/`StringSeq` in corelib-cpp;
the statically bounded profiles (`c`, `cpp` with
...
**Dart's version of that gate is the destination itself.** corelib-dart's
`onBytesDest`/`onArrayDest` defaults used to allocate a destination sized from the
wire, which is right for a hand-written visitor and wrong for a schema-bound scope,
so every generated visitor overrode both with `return null` for every id it did not
bind. Since corelib-dart#96 the header call that announces the field is the one
that returns its destination, and its default is `null`: an id the scope does not
bind, or one whose wire kind contradicts what it binds, gets no destination at all,
structurally, and the generated visitor emits no stub (generator#570). That is a
...
| **TypeScript** | every wrapper array's element **index**, and a `string`/`blob` element's **length**, on the collector that receives those headers instead of the visitor — `StringSeq`/`BlobSeq` (`receiverCap`/`receiverElemMax`) for a leaf element, `ElementSeq`/`FramedSeq` (`receiverCap`) for a framed one (generator#587) | the other kinds, in the flat visitor's own `fixlenBegin`/`arrayBegin` — the hooks that already carry the schema bound, so the cap is its `else`; corelib-ts is handed no limits object at all |
| **Rust** (std) | every wrapper array's element **index**, on the `sofab::seq` call that grows the container — `place_elem` for a string/blob element, `reserve_elem` for a struct/union element, `reserve_row` for a matrix or wrapper row, and `check_index` at the length word and a native row's header — each handed one `Bound`: `Schema(count)` or `Cap(MAX_DYN_ARRAY_COUNT)` (§9.5.2) | string and blob lengths, native array counts and a matrix row's own element count (§9.5.2: the one corelib call on those paths, `acc.feed`, comes one callback too late) |
...
and is known only at the end. Rust std places each element with
`sofab::seq::place_elem` (push defaults up to the id), so a `count: 5` `Vec<String>` goes 0 → 4 → 8, a
...
- **Java and Kotlin's bulk offer widened as a consequence.** `Visitor.arrayBulk`
  ...
  declaration implies — generator#516. Both answered that at the root instead and
  now back the two kinds at the implied width, so the offer covers them again.)
...
**A boolean is the one kind with no width to ride the entry**, and it takes a
binder of its own for it. CORELIB_PLAN §4.4 gives a boolean no wire type and no
declared width at all: it travels as an unsigned integer, an encoder MUST write
`true` as `1`, and a decoder MUST read **every** value other than `0` as `true` --
normalized away, so a re-encode emits `1`, and never INVALID however wide the
varint is. So a boolean binds through corelib-py's `boolean` (scalar) and
`boolean_array` (elements), the read kinds corelib-py#158 added, and neither entry
is given a `max_value` or an `elem_max` to carry: `bindScalarWidth` and
`bindElemWidth` name `KindBool` in the same arm as the floats and return before any
range is looked up, and `ir.NarrowRange` would answer `ok=false` for it anyway
(generator#590).

That the array half is a **separate binder rather than `unsigned_array` without a
width** is the point, not an implementation detail. `unsigned_array` HAS an
`elem_max`; binding a boolean through it with the ceiling its one legal pair of
values suggests makes `256` INVALID, which §4.4 forbids — and that is #581, found
in the C++ backend, where an `array of boolean` stored through a width-limited
unsigned element made a sender's `true` decode as `false` with the outcome still
COMPLETE. `boolean_array` takes no width argument, so the mistake is out of reach
here rather than merely unwritten.

The mapping lives in the corelib on the **bound** route only. The Visitor is
deliberately unchanged: `on_unsigned`/`on_unsigned_array` still hand over the RAW
value, because without a table the decoder cannot know a field is a boolean. A
boolean array reaches that route whenever it is not bound — a declared count over
32, no declared count, or a scope that is not bindable — so the generated
`[bool(_v) for _v in value]` there is what applies §4.4 for those fields and stays.
On the bound route the scatter's `U[at] != 0` no longer normalizes anything; it
CONVERTS an `int` slot to the `bool` the field is. It stays `!= 0` rather than
`== 1`, which is correct against the binder and would silently turn a
non-canonical `true` into `false` if that ever stopped holding. The two spellings
are INDISTINGUISHABLE at runtime while the binder normalizes -- no round trip,
conformance driver or shared vector can separate them -- so the spelling is pinned
in `binding_test.go`, which is the only place that can see it.

Measured (`tests/bench`, both engines, same corelib-py on either side): routing a
boolean array through the corelib costs decode **+5.6 Ir per element** on the
native engine and **+413 Ir per element** on the pure one -- the bound read still
converts `int` to `bool`, so this route got marginally dearer, and the gain is
structural rather than a speed-up. Encode goes the other way, the generated 0/1
list comprehension having gone: on a message dominated by an `array<boolean,
count: 32>`, **−38.8%** native and **−31.0%** pure. Across the whole
`vehicle_telemetry` bench row, whose boolean array is four elements among thirty
fields, that nets out at −1.27%/−0.35% encode and +0.27%/+0.12% decode.
...
**The element index went to the corelib too (generator#587), both bounds included.**
A wrapper array's index is the one bound the codec cannot see, and the backend used
to emit it inline — `if fld.id >= 4: raise SofaDecodeError(...)` or
`>= MAX_DYN_ARRAY_COUNT: raise SofaLimitError(...)`, then a
`while len(_t) <= fid: _t.append(...)` gap fill — at every wrapper array. That is
code with the same shape for every schema, so it is now three corelib-py functions
in `sofab.collectors`, each taking the schema `count` (or `UNBOUNDED`) and the
receiver cap as two exclusive arguments, bounding **before** it grows:

| element | call | emitted in | gap value |
|---|---|---|---|
| `string` / `blob` | `reserve_leaf(out, id, "" / b"", count, cap)` | `on_field`, at the element's header | the shared immutable default |
| `struct` / `union`, native matrix row | `reserve_elem(out, id, Elem / list, count, cap)` | `on_sequence_begin` (struct/union), `on_field` (row) | a fresh `make()` per slot |
| wrapper row | `reserve_row(rows, id, count, cap)` | `on_sequence_begin` | a fresh list; the row at `id` is rebound (§7.4) |

A leaf is *reserved* at its header rather than *placed* at its value, so the index
verdict still lands before the payload is read (§5.2: INVALID outranks INCOMPLETE)
and each element costs one corelib call; the value hook is left a plain
`out[fid] = value`. What stays generated is per-schema routing: the §7.3 decline in
front of the call, the field-level `= []` replace, the `_ixN` registers and the
scope stack. The cap argument is `MAX_DYN_ARRAY_COUNT` where the schema has an
unbounded array, else the configured number as a literal — the same rule
`capsArgs` follows, and there the corelib never compares it. Python has no
compile-time constant form, so the count is a literal argument (`LOAD_CONST`).
The old child-visitor collectors (`StringSeq`, `NestedSeq`, …) were deleted rather
than kept beside the functions: a flat visitor has no child to hand back, and a
held collector instance measured slower per element than a plain function.

This was the one target where the move adds a CPython-level call per wrapper
element, so both engines were measured (Callgrind, subtract, the `python` /
`python-native` rows on `vehicle_telemetry`, the same corelib-py build on both
sides): **pure** decode 1,941,506 → 1,951,946 Ir/op (+0.54 %), **native** 433,632 →
432,798 (−0.19 %, noise). The native engine pays nothing because corelib-py's
`_speedups` carries compiled twins of the three functions, re-exported whenever the
native engine is active; the pure functions alone cost it +2.5 %.

#### 9.5.2 Rust: which caps stay in the generated visitor
...
**Rust std had none, and for three of its four kinds still has none.** The generated
...
The collector shape Go and Dart use hands the corelib the *destination* to HOLD —
`sofab.NewStringSeq(&m.Warnings, bounds, caps)`. Its Rust equivalent, a collector
object kept across callbacks, would be `&mut self.m.warnings` alongside the
`&mut self.m` the visitor already holds: two overlapping mutable borrows, which the
language rejects. A **free function** that borrows one field for one call does not
alias, and that is what `sofab::seq` is (generator#587): `place_elem(&mut
self.m.warnings, id, Bound::Schema(4), s)` borrows the field only for the call. So
a wrapper array's element **index** — the one kind that has a corelib call to ride,
the call that grows the container — is compared in the corelib, against the
`Bound::Schema(count)` or `Bound::Cap(MAX_DYN_ARRAY_COUNT)` generated code passes
(§9.5.6). For the other kinds the argument stands: the one corelib call on a
string or blob's path, `acc.feed(total, offset, chunk)`, sits *after* the point a
cap must fire at; moving the check there would buffer the payload first and
reject it after, which is the eager-allocation §6.2.1 exists to prevent — and a
native array count has no corelib call at its header at all.
...
**Rust std is therefore exempt for those kinds**: its string/blob length and native
count `max_dyn_*` guards stay in the generated visitor, per field, at the
count/length header, ahead of any accumulation. For a
...
deletion mandatory rather than cosmetic: a cap is a guard here or an argument
there, never both.

Three call sites moved, and each already owned the header word the cap must fire
at:
...
`long` at first, with a negative sentinel for the schema case — so a call that had
*forgotten* to state a cap and a call asserting *the schema bounds this field*
...
one: the **destination gate** at the top of `string()` and `blob()`, which
returns for every payload this message does not read. The cap rides that switch —
one store on a dispatch already there, no dispatch of its own — and collapses to a
bare constant when every destination of the kind is schema-unbounded. An arm the
gate falls through to for a `maxlen`'d field leaves `_lim` at its
`Bound.SCHEMA_BOUNDED` initializer, and an undeclared id or a MESSAGE_SPEC §7.3
wire-type contradiction never reaches the accumulator at all.

That the gate covers **both** payload kinds is what makes §6.2.1's *a skipped
field is never capped* structural here rather than a discipline. Until
generator#436 `blob()` had no gate — the reading was that a blob carries no
encoding and so has nothing to skip validating — and a cap resolved without one
has to name the schema-unbounded destinations in a switch of its own so that an
undeclared id keeps `SCHEMA_BOUNDED`. With the gate, the string shape simply
applies to both: the same defect this family has hit before (generator#410;
corelib-cpp's pre-guard sized a destination in front of `readString` and capped a
field it was required to skip) cannot be reintroduced by an arm that forgets it.
The Java conformance suite pins it end to end: an over-cap payload at an
undeclared id, and one whose wire type contradicts the declared one, both leave
the decode `COMPLETE` — and the skipped-blob row measures that it also allocates
nothing.

**Measured, and it is free.** The `java` bench row decodes `vehicle_telemetry`,
which is fully schema-bounded and emits no cap at all, so the comparison was run
separately on the **unbounded** variant of that schema — same `subtract` method,
same JVM pinning flags, one corelib build, two harnesses differing only in where
the compare lives (the old-shape one re-adds the guards and passes
`Bound.SCHEMA_BOUNDED`, so exactly one comparison runs either way):

| shape | `decode` Ir/op | `encode` Ir/op |
|---|---:|---:|
| guard in generated code | 30,636 | 17,003 |
| cap passed into `PayloadAcc` / `Seq.reserveRow*` | 30,630 | 17,005 |
| | **−0.02 %** | +0.01 % |

`encode` is the control: no cap exists on that path, so its ±0.01 % is this
method's noise floor and the decode figure sits inside it. Every run is affine in
reps to within 0.03 % (the harness gate is 1 %). That the difference is below
what the method resolves is the expected answer — the comparison did not appear
or disappear, it moved one call inwards. What the move buys is the correctness
the table above is really about: the check now sits behind the §7.3 tag test and
ahead of every sizing decision, as a property of the structure rather than of
each caller's discipline.

**One thing the move costs**, recorded because it is visible to users: the
rejection's detail line loses the field name. The corelib is handed a payload and
a number, not a schema, so it reports `string length 4096 above configured limit
1024` where generated code used to report `warnings: string length above
configured limit 1024`. Catching and re-raising to put the name back would wrap
...
| a wrapper array's element **index** (generator#587) | `Seq.placeElem` (string/blob), `Seq.reserveElem` (struct/union/nested row), `Seq.checkIndex` (the element's length word) | `cap`, `rcap` |
...
**The element index went the same way (generator#587), both bounds included.**
A flat wrapper array's string/blob element is placed by `Seq.placeElem`, a
struct/union/nested-row element reserved by `Seq.reserveElem` (an `inline fun`,
...
Measured with the same method as Java's (Callgrind, subtract, the `kotlin` row on
`vehicle_telemetry`), with the generator varied against one corelib build:

| shape | `decode` Ir/op | `encode` Ir/op |
|---|---:|---:|
| literal guards and emitted loops (before) | 33,036 | 17,200 |
| string/blob leaf via `placeElem` only | 32,917 | 17,200 |
| … and the framed element via `reserveElem` | 33,019 | 17,200 |
| … and the length word via `checkIndex`, corelib `checkIndex` 48 bytes | 33,417 | 17,198 |
| same, `placeElem`/`checkIndex` made Kotlin `inline` | 34,007–34,018 | 17,202 |
| **shipped**: all three, corelib `checkIndex` 22 bytes | **32,922** | 17,201 |

The old generator on the shipped corelib reads 33,041, so the corelib change by
itself moves nothing. The whole step comes to **−0.35 %** of decode. The one
expensive move was the length word, and it was a JIT size limit rather than a
design problem. `checkIndex` used to build its exception in its own body (48 bytes
of bytecode), and HotSpot C2 inlines unconditionally only up to 35. At the
`fixlenBegin` arms,
`-XX:+PrintInlining` reported `Seq::checkIndex (48 bytes) too big`, and that
compare turned into a real call. corelib-kotlin-mp now takes one compare and hands
the throw to an out-of-line `refusal` that *returns* the exception (a
`Nothing`-typed call would not have helped: Kotlin follows one with a throw of its
own). At 22 bytes it is inlined at every site, and the cap folds there exactly as
the literal did. Kotlin-level `inline` is the wrong lever on the JVM. It copies the
body into every arm of the generated callbacks, and those callbacks then outgrow
what C2 inlines into the decoder loop.
...
**Wrapper element indices ride the placement call (generator#587).** Placing a
string/blob element and reserving a matrix row have one shape for every schema,
so they are corelib-cs's `Seq.PlaceElem` and `Seq.ReserveRow`; `Seq.CheckIndex`
is the bare bound, for the length word in `FixlenBegin` and for a struct/union
element. Each takes the schema `count` (`-1` where the schema declares none) and
`MaxDynArrayCount` as its last two arguments and compares exactly one —
`InvalidMessage` against the count, `LimitExceeded` against the cap — before the
list grows (CORELIB_PLAN §7.2 item 8). So the generated visitor compares no
element index itself, and every message with a wrapper frame declares
`MaxDynArrayCount` even when the schema counts every array: the argument is
required. A native row keeps the order kind, row id (`ReserveRow`), row element
count.

**The struct/union gap fill is the corelib's too.** A struct or union element
is grown by `Seq.ReserveElem(L, id, static () => new Elem(), N, MaxDynArrayCount)`:
the corelib bounds the index and fills the gap, and generated code keeps only the
routing (binding the element index, switching into the element's scope). The
element's constructor has to travel as an argument, and that is the one thing this
costs, measured on the `csharp` bench row (decode Ir/op, `subtract`,
vehicle_telemetry): the baseline reads 69,603, the shipped shape 70,130 — +0.76 %,
encode flat at 29,166. The JIT inlines `ReserveElem` but not the delegate it calls
once per new element, and `AggressiveInlining` on the helpers changes nothing
(70,131). The `static` lambda is cached by the compiler, so a slot allocates
nothing beyond the element. Spellings that were measured and rejected: a
`where T : new()` overload, 71,508 (+2.7 %), because `new T()` in shared generic
code is `Activator.CreateInstance<T>()`; and keeping the fill generated
(`Seq.CheckIndex(...); while (L.Count <= id) L.Add(new Elem());`), 69,774
(+0.25 %) — cheaper by half a point, but a static helper emitted into every user's
tree (§8, "Never emitted silently"), and the same trade the java, kotlin and rust
backends already made the other way. A shape that would recover the half point —
a static abstract `Create()` on every generated element type, so the JIT sees
through the call — is not taken: it adds public API to every generated class.

**What stays a generated guard, and why.** Array **counts**: corelib-cs delivers
array elements one at a time through the same `Unsigned()`/`Signed()` callbacks a
lone scalar uses, so there is no per-array call to hang a count on, and inventing
one purely to hold a check is the shape the measurements in §9.5 put at 1–2.5
percentage points of decode. A string/blob element's schema `maxlen` also stays in
the arm (the receiver half is `PayloadAcc`'s). A split by field kind is allowed,
and this is C#'s.
...
#### 9.5.6 Rust: `sofab::seq`, one trait over four containers

corelib-rs and corelib-rs-no-std carry the same module, `sofab::seq` (`src/seq.rs`,
byte-identical but for the `alloc` gate), and every wrapper-array site the rust
backend emits calls it (generator#587). The generated code names the bound and
compares nothing: there is no `id as usize >=` left in any configuration, which
`TestRustWrapperIndexBoundLivesInTheCorelib` sweeps across all four storage
combinations.

**One `Bound`, exactly one of two.** `Bound::Schema(count)` breaches as
`Error::InvalidMsg`; `Bound::Cap(MAX_DYN_ARRAY_COUNT)` as `Error::LimitExceeded`,
and `Cap(0)` — no cap stated — as `Error::Argument`. An enum rather than a const
generic, so arrays of one container type share one monomorphised copy on the
footprint profile, and a literal bound still folds after inlining. The generated
visitor files the corelib's category through one emitted method, `V::refuse`,
into the sticky flags it already had (`inv`, `lim`, `err`): the category is
decided only in the corelib.

**One trait, four containers.** `SeqVec` abstracts the destination, because the
backend's two switches — `corelib` and `allow_dynamic` — are orthogonal and a
wrapper array can live in `Vec<T>`, `alloc::vec::Vec<T>` or `heapless::Vec<T, N>`
on either corelib. The one difference between them is that a fixed container can
be full, so `grow_to` returns `bool`; `Vec` returns `true`, which folds away. That
replaces the "push can fail" loop the backend used to emit on every fixed-field
profile. A full container answers `Error::Argument` (filed as `err`, the same
`BufferFull` the silent drop now reports) — unreachable today, since the heapless
capacity is the schema `count` and the index is checked first. The impls are
behind corelib features the generated `Cargo.toml` enables: `heapless` under
static storage (on either corelib), `alloc` for `rs-no-std` with
`allow_dynamic: true`.

**What moved, and what did not.** `place_elem` (string/blob element),
`reserve_elem` (struct/union element — the corelib grows, the generated code keeps
binding `_ixN` and routing), `reserve_row` (a matrix row or a wrapper row: it
clears, because the array wrapper replaces, §7.4) and `check_index` (the length
word, and a native row's header ahead of its element-count check, so a rejected row
id still disarms the fill before anything is cleared). The native row keeps its
generated `count > M` check and its `reserve_exact(count)`; string/blob length caps
stay generated for §9.5.2's reason; writing a placed string into a
`heapless::String` in place stays generated because it is shared with plain scalar
string fields and is not one of the three signatures.

**Measured** (`tests/bench/run.sh --rows` for all five rust rows, before vs after,
against the same corelib checkouts):

| row | before | after |
|---|---|---|
| `rust-rs` decode Ir | 22,514 | 22,674 (+0.71 %) |
| `rust-rs-static` decode Ir | 16,080 | 16,134 (+0.34 %) |
| `rust-rs-unbounded` decode Ir | 20,845 | 21,166 (+1.54 %) |
| `rust-rs-no-std` decode Ir | 30,045 | 30,031 (−0.05 %) |
| `rust-rs-no-std-dyn` decode Ir | 38,495 | 38,546 (+0.13 %) |
| `rust-rs-no-std` thumbv6m `.text`/`.data`/`.bss` | 9,913 / 0 / 0 | 9,881 / 0 / 0 |
| `rust-rs-no-std-dyn` thumbv6m `.text`/`.data`/`.bss` | 11,641 / 0 / 4 | 11,705 / 0 / 4 |

Two corelib spellings were settled by these numbers, not by reading the code.
`Vec` grows with a push loop, not `resize_with`: ids arrive in order, so growth is
usually one slot, and `resize_with`'s setup cost `rust-rs-unbounded` +2.6 %. And
`check_index` is one flat match: a shared `(limit, verdict)` helper folded to the
same instructions but pushed the generated `fixlen_begin` over LLVM's inline
threshold, +3 % on `rust-rs-no-std-dyn`. What is left on the maxspeed rows is
`reserve_elem` on a struct element: the same checks spelled inline measured ~300 Ir
cheaper on `rust-rs`, and `#[inline(always)]` wins that back on `rust-rs` while
losing it on `rust-rs-unbounded`, so the corelib keeps `#[inline]`.
...
reaches further than the check it was added for: it was the flag constant being
*emitted at all* that exposed it, not the byte count — no leg compiled the fill
project strictly, the two `-Werror` builds in `tests/conformance/cpp/run.sh`
using the *example* schema, which has no wide bitfield. Closing that is now a leg
of its own: every C++ profile compiles the generated `fill.hpp` under `-Wall
...
|---|---|---|---|
| **C** | `corelib-c-cpp` | descriptor-table callback | `object.h` struct + static descriptor; a `boolean` is `FIELDTYPE_BOOLEAN` / `_ARRAY_BOOLEAN` (§4.4 normalizing read); `symbol_prefix`; auto capability + API-version guards; analytic `MAX_SIZE`; project mode also emits `Makefile` + `CMakeLists.txt`, `run.sh`, and a devcontainer. |
| **C++** | `corelib-cpp` (default) / `corelib-c-cpp` (`corelib: c-cpp`) | child-visitor / flat-visitor wrapper | header-only `OStreamMessage`+`IStreamMessage`; decode on the `cpp` leg goes through the `sofab::read`/`readString`/`readBlob`/`readArray` **helper layer**, which sizes the destination the codec then fills (§6.6, §9.3 family 4); a boolean array's element is `std::uint8_t` and an enum array binds through `sofabgen::RawArray` on **both** legs, because neither a deferred nor a resumable decode may land in a temporary; a boolean array binds through `sofabgen::RawArray<…, bool>` too, so the corelib reads it under §4.4; `c-cpp` decode pre-sizes varlen fields + links the C sources. |
| **Rust** | `corelib-rs` (default) / `corelib-rs-no-std` (`corelib: rs-no-std`) | flat-visitor location-stack | std (throughput, no features) vs no_std (feature-gated, footprint); feature-clean codegen. Field storage is a SEPARATE axis from the environment: `allow_dynamic` selects `String`/`Vec` or fixed-capacity `heapless` on **either** corelib, so a std crate can hold its bounded fields inline while keeping serde and the ordinary std prelude. The std decode scope stack is a fixed `[_Loc; D+1]` (D = the schema's deepest frame) on either storage, so an `allow_dynamic: false` decode touches no heap; `no_std` keeps its `heapless::Vec` sized from the frame count. String/blob chunk reassembly is the corelib's — `sofab::PayloadAcc`, and `PayloadAcc<MAX_SIZE>` on no_std where the storage is the caller's and an over-long split payload is `Error::BufferFull` (generator#345). Wrapper-array placement, growth and index bounds are the corelib's too — `sofab::seq` over a `SeqVec` trait spanning `Vec` and `heapless::Vec`, with the bound passed as `Bound::Schema`/`Bound::Cap` (§9.5.6, generator#587). |
| **Go** | `corelib-go` | push child-visitor | struct implements `sofab.Visitor`; exported `Serialize(*sofab.Encoder)` + `EncodeTo(io.Writer)` — generated code owns every encode buffer (§5.1): `Encode` allocates one exactly-sized `<Msg>MaxSize` buffer (`NewEncoderBuffer`) for a bounded schema and drains a fixed 512-byte scratch into a caller `[]byte` (`NewEncoderSink`) for an unbounded one, and `EncodeTo` drains that same scratch into the writer, so a message never exists as one contiguous `[]byte`; every one of those encoders is constructed with `sofab.WithMaxDepth(<Msg>MaxDepth)` — the deepest sequence nesting the marshal can open, walked from the IR exactly as the Encoder counts open frames (struct/union field +1, wrapper array +1, struct/union element +1 more, native array/row 0) and passed through a package-level `[]sofab.Option` so the call allocates nothing — which lets the corelib size its lazy-sequence id stack (§6.0.1) to the schema, inline in the Encoder, instead of a 1 KB `MaxDepth` allocation per one-shot encode; ONE decode surface (§5.3.1) since corelib-go#130 retired the pull `Decoder` — `Decode<Msg>` via `sofab.AcceptBytes` for bytes in hand and `Decode<Msg>From(io.Reader)` via `sofab.NewDecoder(m).FeedFrom(r, scratch)` for a byte stream are both one resumable `Feed`, so the chunked and one-shot paths cannot drift (memory bounded by the 4 KiB scratch plus the largest single field, §5.6); **no generated `feed()`** yet, though the corelib now has the push decoder one needs; the codec builds NO aggregate (§6.6.3), so a string/blob arrives in pieces (`String(id, total, offset, chunk)`) and is assembled by a `sofab.PayloadAcc` the object carries — one per object, since a fixlen payload is contiguous and two fields are never in flight at once — while a native array arrives as `ArrayBegin` / one `Array*(id, index, v)` per element / `ArrayEnd`, with the destination opened at the header (a repeated id REPLACES, §7.4) and each element width-checked as it lands (§5.2: an over-width element behind a truncation stays INVALID); every generated destination COPIES, so a decoded message owns its bytes and outlives the input buffer; a struct/union type whose members declare, at any depth, a default other than Go's zero value carries an unexported `setDefaults()`, called by `New<Msg>` per such field and handed to `sofab.NewMessageSeqInit` for an array of it, so a fresh message, an absent nested member and an omitted interior element all read as the schema's defaults (generator#609, corelib-go#147); the object embeds `sofab.StringCheck`, so `WithStrictUTF8` reaches the destination's UTF-8 check (§6.4) rather than only the build tag; `BeginSequence` descends into nested objects / array collectors; the receiver-side `max_dyn_*` caps are applied per field, at that field's own count/length header (§9.5): as the *else* of the schema bound in the generated `FixlenBegin`/`ArrayBegin` for a scalar and a native array, and — for a wrapper array's element index, element length and matrix-row count, none of which reach the visitor — inside the collector, which takes them as a `sofab.Caps` constructor argument beside the schema's `sofab.Bounds`; canonical-JSON tags. |
| **Python** | `corelib-py` | flat visitor, `(location, id)` dispatch | dataclasses + `encode()` / `serialize(Encoder)` (public since generator#239); ONE decode surface (§5.3.1) since corelib-py retired the pull decoder — generated `decode(data)` is literally one `Decoder(...).feed(data)`, the same resumable path `decoder(reassembly=…)` → `feed` drives, so the one-shot and chunked paths cannot drift and no `deserialize` is emitted at all; generated code owns every encode buffer (§5.1): `encode()` allocates one exactly-sized `MAX_SIZE` `bytearray` (`Encoder.over_buffer(buf, 0)`) for a bounded schema and drains a fixed 512-byte scratch into a caller list (`Encoder.over_buffer(scratch, 0, out.append)`) for an unbounded one, so `Encoder()`/`getvalue()` — the corelib installing and growing its own storage — is gone from generated code; the schema `count`/`maxlen` is *declared* to the corelib in `on_schema_bound` and the receiver-side `max_dyn_*` caps are *stated* to it as `Decoder(...)`'s three required arguments, which it compares at the count/length header; the reassembly buffer is a **fourth** required argument of the same kind (corelib-py#139) — the size decides which well-formed messages a receiver can stream, so §6.2.1 leaves the codec none to invent, and the backend derives it: `MAX_FIELD_SPAN` is the largest single CONSTRUCT the schema can carry (one string/blob payload, or one native array's element run, with each cap standing in for a missing schema bound), which is *not* C++'s summed field span — a sequence is read field by field and a wrapper array element by element, and here the number is an allocation rather than a refusal ceiling, so over-estimating costs memory; `decoder()` adds room for one fed chunk on top and takes a `reassembly=` of its own for a caller streaming larger pieces, while the one-shot `decode()` needs only the construct in flight; a field the receiver SKIPS enters the buffer at no size at all; `on_field` declines a mistyped header (§7.3) and any id the scope does not declare, which is what keeps the cap off a bounded, unknown or mistyped field, and the one bound the codec cannot see — a wrapper element's INDEX — is handed, with the schema `count` and the cap, to corelib-py's `reserve_leaf`/`reserve_elem`/`reserve_row`, which bound it and grow the list, so generated code emits no index test and no gap-fill loop (§9.5.1, #587); every decoded field COPIES, so a decoded message outlives the input buffer; a Python float is a double, but generated code overrides no raw-bits hook — `write_float32`/`on_float32` carry the ordinary widened channel and corelib-py preserves a signaling NaN bit-for-bit on it by hand (§4.6/§6.5), which the shared `tests/conformance/lib/check_fp32_nan.py` pins across re-encode over the harness's `recode` verb — wire → object → wire, emitted here since #468, the same verb ts and dart run — and `tests/conformance/python/fp32_nan_check.py` pins on both engines, on the materialized value (§6.5's second oracle) and on the streaming surface, through the public `decode()` and through `feed` at several chunk splits (#414); a `boolean` binds through corelib-py's own `boolean`/`boolean_array` read kinds and an array of them encodes through `write_bool_array` (corelib-py#158, #590), so CORELIB_PLAN §4.4's mapping is the corelib's on every bound route and generated code neither normalizes nor canonicalizes a boolean itself — the unbound array keeps its `[bool(_v) for _v in value]`, the Visitor being handed raw values by design — and `tests/conformance/python/bool_tolerant_check.py` drives the shared `boolean_tolerant` vectors through all three routes, on both engines and both decode surfaces. |
| **TypeScript** | `corelib-ts` | flat visitor + static scope map | classes + `serialize(os)` plus `encode()` — generated code owns every encode buffer (§5.1): `encode()` allocates one exactly-sized `MAX_SIZE` `Uint8Array` (`new OStream(buf)`) for a bounded schema and drains a fixed 512-byte scratch into a caller list for an unbounded one, where the sink is handed the INSTALLED buffer plus the region's coordinates (`(_b, _s, _e) => _out.push(_b.slice(_s, _e))`, §5.1.6 — a `subarray` would be an allocation per flush) and the corelib's own `growingOStream()`, which owns and doubles a slab of its own, is emitted nowhere; ONE decode surface (§5.3.1): `decode(bytes)` runs the corelib's `decode` against a per-type flat `Visitor`, and `decoder()` → `feed`/`finish` drives the very same visitor over the resumable `IStream`, so the chunked and one-shot paths cannot drift; dispatch keys on `(location, id)` with the parent restored from a static `switch` rather than a stack (the scopes of a type form a tree, and a declined subtree fires no `sequenceEnd`); the schema-free half is the corelib's — `PayloadAcc`, the four wrapper-array collectors (`StringSeq`/`BlobSeq` for a leaf element, `ElementSeq`/`FramedSeq` for a framed one, the latter taking a generated factory for a gap value that must be fresh per slot), `decodeUtf8`, `elementsEqual`/`longElementsEqual` and `fp32RawBytes` are called, not re-emitted (#345 on corelib-ts#151, #587), so a generated module declares no top-level `function` at all and the `_ObjSeq`/`_MatSeq`/`_RowSeq` collectors the withdrawn child-visitor shape needed are gone; 64-bit → `bigint` by default, `int64: long`/`number` backs u64/i64 arrays with corelib `Long[]` accessors — and the scalars too, as `Long` under `long` or `number` under `number` — built from the hooks' `lo`/`hi` wire halves (`Long.fromBits`), so the hot path materialises no `bigint` and needs no opt-in channel (corelib-ts#161 withdrew `Visitor.longs`, whose per-schema trade #344 had to guess); a narrow destination reads the hook value as a `number` with no conversion call, the declared-width guard on the next line being what makes that assertion true; a **bitfield** takes the same narrowest-carrier rule the table's common mapping states, with the two carriers this language has — a flag at position 31 or above makes the field a `bigint` (masks emitted as a literal-typed `const` object — `as const`, not `Object.freeze` — since a TS `enum` member can only be a number, and JSON-carried as a decimal string), everything below it stays a `number` enum; the boundary sits one position BELOW the carrier's storage limit because JavaScript narrows both operands of `|`/`&` to 32-bit SIGNED, so a mask with bit 31 set comes back negative and the encoder refuses it — and it is NOT an `int64` mode: a mask has no lossy-number reading (#470); alloc-free `writeString`; native arrays are typed arrays filled in place through `arrayBulk` (an `fp32` array through the bit-exact `f32` destination, corelib-ts#188 or later), and every EMPTY one is the module's shared zero-length `_E_<Type>` instance, so two fresh messages' empty array members are the same object (§9.3); a `number` is a 64-bit double, so an fp32 NaN keeps the hook's 32-bit wire word in a `Uint8Array \| null` companion, captured only for a NaN, to preserve a signaling NaN bit-for-bit (§4.6, #235); `recode` harness mode (wire → object → wire) exercises it, driven by the shared `tests/conformance/lib/check_fp32_nan.py` (#468); the receiver-side `max_dyn_*` caps are applied by the generated visitor, per field, at each field's own count/length header (§9.5, #388), and a wrapper array's element index and a leaf element's length — the headers a collector receives instead of the visitor — take theirs as that collector's own `receiverCap`/`receiverElemMax` arguments (#405, #587: the index cap rides on all four collectors, the length cap on the two leaf ones), so the corelib is handed no `DecodeLimits` at all; every decoded value is COPIED out of the fed chunk (§6.7), so a message owns its bytes and outlives the input. |
| **C#** | `corelib-cs` | flat-visitor location-stack (`IVisitor`) | classes + `Serialize`/`EncodeTo`; nested `Msg.Decoder` (constructed with `new`, not a `Decoder()` factory — C# puts nested types and members in one declaration space) → `Feed`/`Finish` for chunked decode; `TryDecode(data, out msg)` returns the §7 `DecodeStatus` (#105). `Encode()` keeps a per-thread scratch buffer AND a per-thread `OStream` that it `Reset`s for each message (corelib-cs#110), so the encoder's fixed state (§6.6: the open-sequence run, sized once at construction) is laid down once per thread rather than zeroed per message, and a message abandoned by an exception leaves nothing behind for the next; the re-entrancy caveat is the scratch buffer's (no `Encode()` from inside a `Serialize()` override on the same thread). The visitor decodes a string/blob that arrives in one chunk inline (the receiver-cap call first, where a configured cap is live) and creates its `PayloadAcc` only for a payload split across feeds; string/blob element placement, struct/union element growth, matrix row reservation and every wrapper index bound are the corelib's `Seq` (§9.5.5); a wrapper-array `List<T>` whose `count` is at most 64 is constructed with that capacity (`new(N)`, still empty — `count` is a capacity, not a length). Integers map to their EXACT declared width, unsigned included (`u8` is a `byte`, `u8[]` a `byte[]`), and so do the two named kinds: a SCALAR `enum`/`bitfield` is the generated `enum X : sbyte`/`: byte` at the width its declaration implies (MESSAGE_SPEC §1), and an ARRAY of either is the bare primitive of that same width — `sbyte[]` for an `array<enum>` over `{0,1,2,10}`, `byte[]` for an `array<bitfield>` over positions 0..3 — rather than the named type, because `OStream.WriteArraySigned`/`WriteArrayUnsigned` take the primitive and a `Gear[]` could only reach them through a converting temporary. Only the `boolean` array stays a `List<bool>`. `IVisitor` offers no bulk element hand-off, so every array element arrives in the 64-bit accumulator and the generated arm carries the §1 width guard ahead of its narrowing cast; System.Text.Json harness (a `byte[]` field, blob or bitfield array alike, goes through the harness's number-array converter rather than STJ's default base64). |
...
| **Dart** | `corelib-dart` | push child-visitor (`MessageVisitor`), one call per field | classes with per-field defaults + `serialize`/`encodeTo`/`encode()` — generated code owns every encode buffer (§5.1): `encode()` allocates one exactly-sized `maxSize` `Uint8List` (`Encoder.overBuffer(buf)`, returning the `written` view over it) for a bounded schema and drains a fixed 512-byte scratch into a caller `BytesBuilder(copy: true)` (`Encoder(sink, buffer: scratch)`) for an unbounded one; either encoder is built with `depth:` the message's `static const int maxDepth` — the deepest sequence nesting `serialize` can open, walked from the IR exactly as the Encoder counts open sequences (struct/union field +1, wrapper array +1, struct/union element +1 more, native array/row 0; a message that opens none passes 1, the corelib's floor) — so the corelib sizes its held-back sequence run (§6.6) to the schema at construction instead of a 1 KiB `maxDepth` run zeroed on every one-shot encode; the corelib's `Encoder.encodeToBytes` — the one place that package allocates output storage — is emitted nowhere; `decoder(out)` → `feed`/`finish` for chunked decode (`finish` returns `null` rather than throwing — this backend's decode path is deliberately exception-free). **Decode writes in place** (corelib-dart#96, generator#570): every string, blob and native array field is one of the corelib's `Inline…` destinations — `final sofab.InlineString`/`InlineBytes`/`InlineInt64Array` (every integer kind, bool/enum/bitfield included)/`InlineFloat32Array`/`InlineFloat64Array`, storage of a fixed capacity plus the `length` in use — and wrapper arrays and matrices hold `Inline…` elements. The visitor answers each aggregate's ONE header call (`onString(id, length)`, `onBlob`, `on{Unsigned,Signed,Fp32,Fp64}Array(id, count)`) with the field's destination after judging the bound there (`invalidate()` for a schema `maxlen`/`count`, §7.1; `limitExceeded()` for a `max_dyn_*` cap on a schema-unbounded field, §9.5); the codec sets `length` and writes the payload into `storage`, and nothing is called when the field is whole — so nothing is copied, no view is built and nothing is allocated per message. A destination whose schema bound fits in 1 KiB (`eagerDestBytes`) is allocated at that bound when the object is built; a larger or unbounded one starts empty and gets storage of exactly the announced count at the header, after the check — allocated once, never grown (§9.5), and never the schema maximum on every fresh `decode()` (an `array<i32, count: 100000>` would be 800 KB). Measured on `tests/bench` (all bounds small): eager decode 40 092 Ir/op vs 40 873 with everything sized at the header. §7.3 is structural twice over: the call is chosen by the WIRE kind and an arm exists only in the call of the declared one, and every corelib default answers `null` (skip) — including `onSequenceStart` — so a scope overrides only what it binds and a skipped field gets no destination at all. The declared element width rides on the destination (`range: const sofab.ElemRange(lo, hi)`) and on `sofab.IntMatrixSeq`'s `lo`/`hi`, applied by the codec per element on both surfaces (§5.2, #267, #516); UTF-8 is validated by the codec for a bound string and by `writeStringUtf8` on encode, so generated Dart carries no UTF-8 code. Encode writes each destination's `storage` and `length` uncopied (`writeStringUtf8`, `writeBlob`, `write*Array(id, storage, length)`); a bool array stores the wire's integers, is compared with its default as booleans and is normalized to 0/1 in place (`_bools01`) before it is written, so a decoded non-zero element re-encodes canonically; a declared string/blob/array default is a `static final` typed list on the class (`_<field>Default`), so filling it at construction and in `reset()` is a typed memmove and the omission test (`_prefixEq`/`_boolsEq`) reads only the `length` in use. `reset()` sets lengths back and clears wrapper lists, keeping every byte of storage, so `tryDecode` into a reused object allocates nothing; `int` is 64-bit so a u64 >= 2^63 is emitted as its signed/hex bit pattern; a `double` is 64-bit so an fp32 NaN routes through the corelib raw-bits API (`onFp32Bits`/`writeFp32Bits` with a companion `int?` slot for a scalar; an fp32 array's `Float32List` storage holds the raw bits) to preserve a signaling NaN bit-for-bit (§4.6, #226), exercised through the `recode` harness mode by the shared `tests/conformance/lib/check_fp32_nan.py` (#468); `tryDecode` -> `DecodeStatus` (`decode` is the best-effort convenience); a decoded message owns its bytes on BOTH paths because every payload lands in the object's own storage (`tests/conformance/dart/ownership_check.dart`); the collectors are the corelib's (`sofab.StringSeq`/`BlobSeq`/`MessageSeq`/`NestedSeq`/`IntMatrixSeq`/`Float32MatrixSeq`/`Float64MatrixSeq`, §8, #345) and take a wrapper array's element index, element length and matrix-row count caps as their `rcap`/`relemMax`/`rowCap` **required** arguments; JSON harness carries u64 as a string. |
...
and never the validity bound (both kinds are bounded by the WIDTH their
declaration implies, which the narrowest backing happens to coincide with; see
"Decode verdict" above, and note the targets that carry an enum or a bitfield at
full width are making the same MAY the other way and must then state the bound
as an explicit check); a counted
...
what the field carries. The declared-width guard is what bounds it, not the
...
1. **The shared conformance file** — each corelib ships
   ...
   *Nested defaults* (`tests/conformance/lib/check_defaults.py`, generator#609):
   ...
   struct array's element — is what absence means (MESSAGE_SPEC §2/§5.1). The
   driver asserts wire facts and takes every expectation from its OWN schema: a
   ...
   the zero value while `example.yaml` declared `nestedint: 255` the whole time.
   ...
   *Enum / bitfield declared width* (`tests/conformance/lib/check_declared_width_kinds.py`):
   ...
   by the smallest unsigned type holding its highest `pos` — so a wire value
   outside that width is `INVALID` (§7.1) however wide the integer the target
   stores the field in, and a value inside it decodes even where the schema names
   no constant for it (generator#516).
   ...
   implying an `i8`, and the bitfield positions `0, 1, 3`, implying a `u8`. The
   gaps are what the ACCEPTING rows are made of: `5` sits between the constants
   and `4` sets a bit no flag declares, and both MUST decode. A decoder still
   carrying the withdrawn set/mask reading fails exactly those two rows and passes
   everything else here, which is what makes the gaps worth keeping.

   Each kind is probed with several classes of value — a **declared** one
   (accepted, and its value read back), an **undeclared one inside the implied
   width** (accepted), one **past that width** (rejected), **both edges of the
   width** (accepted, which is what separates a real width check from one taken
   from the constants' hull `0..10`), and, for the enum, one **below the width**
   (rejected: an enum array is a SIGNED array on the wire, so a bound that lost
   its lower half, or a hook handed only an `elem_max`, would pass every other row
   here) — at all **six positions** a value can land in: scalar field, native
   array element, struct member, member of a struct-array element, union member,
   matrix row element. The six matter because most backends emit one store arm per
   kind serving the four scalar-family positions and a second pair for the two
   ...
   a regex the harness's own output must match — and the driver REFUSES to run
   with neither. Exit status alone scores a panic, an assertion failure or a
   safety-checked process abort as a correct rejection, which is precisely the
   failure this rule's zig half existed to remove (generator#517: an out-of-range
   `@intCast` aborting with rc=134 instead of answering a verdict), so the check
   could not have seen a regression back into it.
   The accepted rows include the zero value of both kinds, every declared bitfield
   combination and both edges of each implied width. Every payload is complete, so
   ...
   **Every row runs again, cut into chunks.** The one-shot verb hands the whole
   ...
   single-engine run proves the rule for one of them.

   What the driver does NOT forge, at any target, is a truncated payload: every
   one it writes is complete, deliberately, so that a rejection can only be this
   rule. The truncated half is covered by construction instead — the corelib hooks
...
2. **Round-trip harness** — `emit: project` builds the generated code against the
   ...
   **malformed** input — an over-count scalar array (5 elements against
   ...
   `over_buffer`/`_put`/`_drain` code. Every leg that reaches a corelib rule
   implemented twice is in that loop for the same reason, and the §4.4 boolean
   leg (`bool_tolerant_check.py`, #590) is the newest: the bound read kinds
   `boolean`/`boolean_array` normalize in the pure decoder's own pass and in the
   accelerator's, and `write_bool_array` exists once in each, so a single-engine
   run would leave half the library a user actually gets untested. A cloned corelib has no built extension,
   ...
   **A decoded message owns its bytes.** CORELIB_PLAN §6.7 — no value the codec
   ...
   Dart, the port that set this discipline, is now the case where the property
   holds at *two* layers at once — corelib-dart copies each payload into the
   destination `onBytesDest` supplied, and the generated destination copies again
   — so mutating either layer alone still passes and only removing both turns it
   red (measured). That is the right answer for a property test, and it is stated
   in the file rather than left to be read as coverage of the generated side.
   ...
   **An `fp32` payload survives decode bit-for-bit — into the value, and back
   out onto the wire.** CORELIB_PLAN §6.5 states the outcome universally — the
   exact 4 wire bytes, signaling NaNs included, at a scalar `fp32` and at every
   element of an `fp32` array — and the §6.5 table assigns only the *obligation
   to build a raw path* to the targets whose one float type is a double:
   **TypeScript, Python, Dart**. Widening an fp32 sNaN into a double is where
   the payload dies, because IEEE widening **sets the quiet bit** while keeping
   the payload (§6.5's diagram: `0x7F800001` → `0x7FC00001`) and no later code
   can recover the signaling-ness. The oracle is wire → object → wire with **no
   JSON detour**, because JSON renders every NaN the same and cannot tell a
   signaling one from a quiet one — measured on python, where `decode | encode`
   returns the *payload-less* canonical quiet NaN `0x7FC00000`, a second and
   different loss.

   Each of the three pinned this after a real defect and each wrote its own
   block — ts and dart as shell loops over their harness `recode` verb (#235,
   #226), python as an in-process driver (#414), because
   `generators/python/project.go` emitted no `recode` verb at all — and no two
   of the three tables agreed: dart had no control value and no §2 row, ts had
   no quiet NaN at an array position, only python had both §2 rows. One shared
   driver now carries the concern for all three (#468):
   `tests/conformance/lib/check_fp32_nan.py` takes the harness argv after `--`
   the way `check_chunk_invariance.py` does, and python's harness emits the
   `recode` verb the other two already had. Every fixture byte is **derived from
   the suite's own schema** — the field's id becomes the header varint, the
   element width and fp32 subtype the fixlen word, the array field's declared
   default the §2 row's payload — by a narrow scan that reads both `example.yaml`'s
   block style and the one-line flow style the ts and dart suites write their
   `conf.yaml` in, a list default in either of *its* two spellings (inline and
   YAML block list), and hard-fails on a `default:` key whose value it cannot
   read: returning "no default declared" there would swap the §2 row for a
   weaker one and keep the suite green, so the reformat of a schema must not be
   able to delete an assertion. Nothing is hard-coded, and a wrong id fails the
   exact rows because they carry the very same header. Each suite also
   contributes a different **encode arm** to the same table: ts's and dart's
   `vecf32`/`vecf32a`/`vecf32ad` are bounded, so the re-encode runs through the
   exactly-sized `MAX_SIZE` buffer, while python's `example.yaml` is unbounded
   and drains a scratch through a sink.

   §6.5's testing clause names **two** oracles — "across decode → re-encode
   **and** any materialized walk" — and calls the second one out by name: "a
   port that guards its round-trip path but not its value path is the defect
   class this section exists to prevent". A wire → wire verb reaches only the
   first, so the second stays where a target can express it:
   `tests/conformance/python/fp32_nan_check.py` requires the materialized
   `msg.somefp32` / `somefloatarray[i]` to be the fp64 image of the wire word,
   sign, quiet bit and 23-bit payload intact. It also drives the **streaming**
   surface — §6.5 asks for the outcome on *every* decode surface, and `recode`
   is one-shot in all three harnesses — at chunk widths 1, 3 and 4096, where 3
   splits the 4-byte fp32 word rather than landing beside it. (That sweep cannot
   move to `check_chunk_invariance.py` either: that driver compares parsed JSON
   values, and two separately parsed NaNs are never equal in python.) It runs
   inside the engine loop and re-asserts `sofab.IMPL` in its own process,
   because corelib-py's pure and native halves narrow and widen fp32 with
   separate code and the shared driver — a subprocess inheriting
   `SOFAB_PUREPYTHON` — selects an engine but cannot assert one. It **imports
   the shared fixture table** rather than carrying one, so the rows python
   checks and the rows the family checks cannot drift apart again. The two
   oracles coincide in python today — the dataclass value *is* what the
   re-encode narrows — and stop coinciding the moment a target stashes raw bits
   beside a canonical value, which is exactly what §6.5 tells a double-only port
   to build.

   Two more things the ports state rather than imply. Carrying raw wire bytes
   must **not** be read as "the field was present": MESSAGE_SPEC §2 decides
   presence from the value, so an explicitly-encoded `+0.0` — and an
   explicitly-encoded array default — must still normalize away to the empty
   message on re-encode, which is the regression a raw-bits path introduces and
   which all three suites now assert at a scalar. At an array position the row
   is derived from a **declared** array default, so it needs a field that has
   one: `example.yaml`'s `somefloatarray` is that field in python, and ts and
   dart declare `vecf32ad` — the same `fp32[3]` as `vecf32a` but with a default
   — beside their existing subject, so all three now carry the array §2 row.
   That row matters most exactly there: ts and dart are the two ports keeping a
   per-**array** raw-bits companion (a `Uint8Array | null`, a bit-exact
   `Float32List` copy), which is the code most likely to derive presence from
   "raw bytes were captured" rather than from "value ≠ default". Beside it, and
   independent of it, ts and dart keep the **inverse** row on the
   default-less `vecf32a` — an all-zero `fp32[3]` is not the empty default and
   must re-encode **verbatim**, count word included (measured green in both).
   Because those two rows can substitute for each other, each suite pins the
   number of rows of **each kind** (`--expect` *and* `--expect-normalize`), not
   just the total. And the python leg pins an
   **outcome, not a mechanism**: generated python overrides no raw-bits hook
   (`backend.go` emits `write_float32`, `visitor.go` routes `on_float32`), so
   what holds the bits together is corelib-py's hand-written widening on the
   ordinary path, and on x86-64 even *that* is invisible — bare `struct`
   round-trips an sNaN unharmed because SSE moves do not quiet, so deleting the
   manual path stays green here and would only fail where the platform
   normalizes (measured, three ways: promoting a signaling NaN to quiet in the
   **emitted encode path** turns each suite red on the shared driver and on
   nothing else — 2 rows in ts and dart, where the mutation can be exact, and 4
   in python, where a NaN can only be quieted by replacing it wholesale; the
   same promotion in the emitted **decode** path turns the python driver's value
   oracle red on all four surfaces, 16 value failures beside 16 re-encode ones;
   and a corelib copy whose `_pack_f32_bits` returns `0x7FC00000` turns six
   fixtures red on the re-encode oracle only, the value oracle staying green,
...
9. **Warning-clean generated code** — every build of generated code in a
   ...
   **The C and C++ suites build the whole corpus that way, not only the
   ...
   work dir except a cloned corelib, so a project generated by a block added
   later is covered without naming it, and the hand-written drivers placed in a
   generated module are held to it too. Python has no compiler warnings, so the
   linter stands in for them: pyflakes (F) catches what a compiler would (an
   unused import, an undefined name, a redefinition), and E9 a module that does
   ...
   ResourceWarning raised from a finalizer cannot fail a process, so the
