...
the same cap, in generated code (Rust, Kotlin, Python, C#'s struct/union elements) or in
...
`Seq.PlaceElem`/`ReserveRow` in corelib-cs, whose struct/union element fill stays generated behind `Seq.CheckIndex` (§9.5.5),
...
**The struct/union gap fill stays generated — the §8 maxspeed override.**
corelib-cs does offer `Seq.ReserveElem`, but it needs the element's constructor as
an argument, and each spelling of that costs decode time on the `csharp` bench
row (decode Ir/op, `subtract`, vehicle_telemetry): the committed baseline reads 69,602 (twice). With the fill moved into
`Seq.ReserveElem` behind a `static () => new Elem()` factory it read 70,133
(+0.77 %): the JIT inlines `ReserveElem` but not the delegate it calls once per
new element, and `AggressiveInlining` on the helpers changed nothing (70,131).
A `where T : new()` overload is worse, 71,508 (+2.7 %), because `new T()` in
shared generic code is `Activator.CreateInstance<T>()`. The fill is one line, so
it stays generated: `Seq.CheckIndex(id, N, MaxDynArrayCount); while (L.Count <= id)
L.Add(new Elem());`. The shape that ships reads 69,774 (+0.25 %, encode flat at
29,166), for zero literal index compares.
