...
including the schema-bounded ones §6.2.1 forbids it to touch, and the raise that
kept those decodable did so by loosening the cap for the *unbounded* fields too.
Its guards moved into `on_field`, where the schema is known, and its cap is
therefore emitted as configured — with no raise; pure C++ additionally derives a
