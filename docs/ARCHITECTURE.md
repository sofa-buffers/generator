...
- **`enum` and `bitfield` are bounded by the width their DECLARATION implies, and
  ...
  - **One `go` defect sits next to this rule without belonging to it.** A
    wrapper-array element is materialised by `sofab.NewMessageSeq`, which appends
    a ZERO value, and generated code applies no defaults to it: the canonical
    empty last-element frame decodes a struct-array element to `0` on every
    member, whatever the schema declared. It is pre-existing and independent of
    §1 — reproduced with a plain `u8 default 7`, where `python`, `dart` and
    `typescript` all return the declared default for the identical bytes. Closing
    it means emitting a default-applying constructor for wrapper-array element
    types and a generated collector wrapper that seeds each newly placed element,
    since corelib-go's collector offers no element factory.
