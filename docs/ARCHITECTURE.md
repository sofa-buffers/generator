...
- **Static helpers belong in the corelib; generated code stays thin.** A corelib
  ...
  corelib-java and corelib-kotlin-mp are the shape to copy — one collector in the corelib, the bounds passed per field; and
  ...
  **erased**, so `placeElem` has one body for every element type a schema can
  ...
  Kotlin emits no index compare of its own at all (§9.5.4).
