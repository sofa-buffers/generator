...
2. **Round-trip harness** — `emit: project` builds the generated code against the
   ...
   **An `fp32` payload survives decode → re-encode bit-for-bit.** CORELIB_PLAN
   §6.5 states the outcome universally — the exact 4 wire bytes, signaling NaNs
   included, at a scalar `fp32` and at every element of an `fp32` array — and the
   §6.5 table assigns only the *obligation to build a raw path* to the targets
   whose one float type is a double: **TypeScript, Python, Dart**. Widening an
   fp32 sNaN into a double and narrowing it back is where the payload dies, since
   the usual answer is to quiet it (`0x7F800001` → `0x7FC00000`). The oracle is
   wire → object → wire with **no JSON detour**, because JSON renders every NaN
   the same and cannot tell a signaling one from a quiet one — measured on
   python, where `decode | encode` returns the canonical quiet NaN. ts and dart
   each carry a shell block over their harness `recode` verb after a real defect
   (#235, #226); python has neither the verb nor the defect, so its leg is
   `tests/conformance/python/fp32_nan_check.py` (generator#414), an in-process
   driver over the already-generated example project which also drives the
   **streaming** surface — §6.5 asks for the outcome on *every* decode surface,
   and neither of the other two blocks covers it. It runs inside the engine loop
   and re-asserts `sofab.IMPL` in its own process, because corelib-py's pure and
   native halves narrow and widen fp32 with separate code.

   Two things the ports state rather than imply. Carrying raw wire bytes must
   **not** be read as "the field was present": MESSAGE_SPEC §2 decides presence
   from the value, so an explicitly-encoded `+0.0` — and an explicitly-encoded
   array default — must still normalize away to the empty message on re-encode,
   which is the regression a raw-bits path introduces and which both the ts block
   and this driver assert. And the python leg pins the **outcome, not the
   mechanism**: generated python overrides no raw-bits hook (`backend.go` emits
   `write_float32`, `visitor.go` routes `on_float32`), so what holds the bits
   together is corelib-py's hand-written widening on the ordinary path, and on
   x86-64 even *that* is invisible — bare `struct` round-trips an sNaN unharmed
   because SSE moves do not quiet, so deleting the manual path stays green here
   and would only fail where the platform normalizes (measured, both ways: a
   generated visitor that quiets every NaN turns 5 of the 8 fixtures red on both
   engines, and a corelib copy whose `_pack_f32_bits` does turns the same five
   red on the pure one). The eight native-`fp32` targets
   are untested for the same rule; §6.5 says the natural implementation meets it
   on its own, which is an implementation note rather than a coverage exemption,
   and a `lib/` driver could reach them once more than two backends emit a
   wire → wire harness verb.
