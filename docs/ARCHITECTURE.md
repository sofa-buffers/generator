...
- **`enum` and `bitfield` are bounded by the width their DECLARATION implies, and
  ...
  - **Where the elements are consumed inside the corelib, the bound travels there
    ...
    `i16` row already did; `dart` returns a `sofab.ElemRange` from
    `onArrayElemBound` and arms `sofab.IntMatrixSeq`'s own `lo`/`hi`; `python`
