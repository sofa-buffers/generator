...
| **Rust backend** | **done** — `generators/rust` (corelib-rs-no-std): structs + `marshal` + a flat-visitor decode (location-stack state machine), `require!` capability guards + Cargo features, serde-json harness; **37 shared vectors byte-exact**. A `corelib: rs` target option emits against the always-on std **`corelib-rs`** (no features / no `require!`); the no-std crate enables only the wire types each schema needs (`default-features = false`), so the corpus spans the feature-subset matrix. `tests/rust/run.sh` exercises **both** corelibs. |
