...
**Measured in the devcontainer, by hand — never in CI.** `results.txt` is
regenerated by a person running `tests/bench/run.sh` in the devcontainer, the same
way the benchmark arena is driven. There is no bench CI job, and adding one is a
mistake worth naming: Ir/op depends on the compiler that produced the binary, so a
runner with its own toolchain pins is a second measuring device. Two devices
disagree on unchanged code — a CI bench on `ubuntu-24.04` with Go 1.26 read one row
at 56,237 Ir/op that the devcontainer (Go 1.24) read at 24,698 — and a diff that is
always large is a diff nobody reads. One environment owns the file.
