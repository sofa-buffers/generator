...
| `max_message_size` | c, cpp, rust, go, java, csharp, zig, dart, python | Ceiling on a message's encoded size (default 4096). Fills in for a message the schema cannot bound (emitted as `MAX_SIZE_LIMIT`); when set explicitly it is also a budget a computed worst case may not exceed (§9.6). |
...
- **bounded** — one exactly-sized buffer holds the whole message, handed to the
  ...
  `sofab.NewEncoderBuffer`, Python `Encoder.over_buffer(buf, 0)`). A value the
...
- **unbounded** — `MAX_SIZE` is an imposed ceiling, so it must not size a buffer:
  ...
  `Encoder.over_buffer(scratch, 0, sink)`), which bounds memory by the scratch
