...
| `run_formatter` | `off` \| `auto` \| `require` | `off` | Whether `sofabgen` may run the target's canonical formatter over the files it just generated — `rustfmt` for `rust`, `dart format` for `dart`, `ruff format` for `python`, `prettier` for `typescript`. `off`: no external program is ever spawned, so the output depends on your definitions and this config alone. `auto`: format when the tool is installed, otherwise write unformatted output and note it on stderr. `require`: format, and fail the run when the tool is missing or refuses. The CLI `--format` flag overrides it. Targets whose formatter is a library (`go`) or whose emitter already produces canonical layout (`zig`) are formatted either way, whatever this says. A target with no canonical formatter at all (`c`, `cpp`, `java`, `kotlin`, `csharp`, `docs`) has nothing to run, so the key is inert there, `require` included. It is called `run_formatter` rather than `format` because `format` is the `docs` target's own option (`format: html`), and a generic key may not reuse a per-target name — the two would read as each other. |
...
| `kotlin` | [kotlin.md](kotlin.md) | `package` |
